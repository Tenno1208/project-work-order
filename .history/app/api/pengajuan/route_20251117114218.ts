...
// File: route.ts (Hanya fungsi POST yang dimodifikasi)

export async function POST(req: Request) {
    // ... (Cek URL & Token)
    try {
        // ... (Ambil formData, token, timestamp, dummy data)
        ...
        let results;
        try {
            // JALANKAN PROMISE.ALL TANPA TRY/CATCH INTERNAL (Memaksa error PDAM keluar)
            results = await Promise.all(uploadPromises); 
            results.forEach(url => fileUrls.push(url));
        } catch (uploadError: any) {
            // Jika ada error dari uploadFile, lemparkan error ini ke try/catch utama di luar POST.
            // Ini akan memastikan pesan error PDAM yang dikonstruksi diteruskan ke frontend.
            throw new Error(uploadError.message); 
        }
        
        // --- MEMBUAT DATA UNTUK API UTAMA ---
        // ... (standardFields logic tetap sama)
