...
export default function DataPengajuanPage() {
    ...
    // Tipe itemToDelete sudah benar: {id: number | null, uuid: string | null, hal: string | null}
    ...
    // --- Data Fetching Logic (Dibiarkan sama) ---
    ...
    /**
     * PERBAIKAN: Hapus setTimeout agar navigasi langsung berjalan.
     */
    const handleBuatPengajuan = () => {
        ...
        // Langsung panggil router.push
        router.push("/dashboard/lampiran/tambah");
        
        // Reset creating status setelah inisiasi push
        setCreating(false); 
    };
    ...
    // Fungsi ini sudah benar (menerima uuid)
    ...
    // Fungsi ini sudah benar (menggunakan uuidToDelete)
    const handleConfirmDelete = async () => {
        ...
        const uuidToDelete = itemToDelete.uuid; // Menggunakan UUID
        ...
        if (!token) {
            ...
            setIsDeleting(false);
            setItemToDelete({id: null, uuid: null, hal: null}); // Reset itemToDelete
            return;
        }
        ...
        const deleteUrl = `${DELETE_API_BASE}/${uuidToDelete}`; // URL menggunakan UUID
