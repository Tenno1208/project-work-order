...
useEffect(() => {
  const fetchHal = async () => {
    try {
      ...
      // Jika struktur API-nya mirip yang jenis pekerjaan (ada json.data)
      if (json && Array.isArray(json.data)) {
        const names = json.data.map((item: any) => item.nama_hal || item.nama || item.label || item.hal);
        setHalOptions(names.filter(Boolean));
      }
    } catch (err) {
