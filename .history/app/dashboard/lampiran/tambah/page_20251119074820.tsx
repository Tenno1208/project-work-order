...
export default function LampiranPengajuanPage() {
    ...
    // Dihapus atau diubah agar selalu kosong, sesuai permintaan
    const nomorSurat = ""; 
    ...
    const fetchTtdHistory = useCallback(async (token: string, npp: string) => {
        try {
            ...
            // Panggil endpoint proxy yang SEKARANG MENGEMBALIKAN DATA GAMBAR
            ...
            if (res.ok) {
                // Konversi respons gambar menjadi Blob
                const imageBlob = await res.blob();
                
                // Buat Data URL Base64 dari Blob
                ...
                // Jika sukses, kita hanya memiliki SATU gambar (Tanda Tangan Aktif)
                // Jika ingin mendukung RIWAYAT, API proxy perlu diubah kembali 
                // ke mode JSON yang mengembalikan array, atau kita hanya menganggap
                // ini adalah TTD AKTIF (yang paling baru).

                // Karena proxy Anda sekarang diset untuk mengembalikan SATU TTD AKTIF, 
                // kita hanya mengolah satu gambar ini.
                
                const processedUrl = await makeImageTransparent(base64DataUrl);
                
                // Simpan TTD aktif ke history (agar bisa ditampilkan di modal) dan preview
                setTtdHistory([processedUrl]);
                setTtdPelaporPreview(processedUrl);
                
            } else {
                // Jika proxy mengembalikan 404/error, anggap tidak ada TTD aktif.
                console.warn("No TTD image found via proxy or API failed:", res.status);
            }
        } catch (err) {
            console.error("Error fetching TTD image via proxy:", err);
            // setNotification({ type: 'error', message: 'Gagal memuat tanda tangan.' });
        }
    }, []);
// ... (Sisa file)
