...
export default function LampiranPengajuanPage() {
    ...
    const fetchTtdHistory = useCallback(async (token: string, npp: string) => {
        try {
            // Memanggil endpoint proxy lokal TTD
            ...
            if (res.ok) {
                ...
                if (json.ttd_url && typeof json.ttd_url === 'string') {
                    // Format API: { success: true, ttd_url: "url" }
                    ttdUrls = [json.ttd_url];
                } else if (json.data && Array.isArray(json.data) && json.data.length > 0) {
                    // Format API: { success: true, data: [{path: 'url1'}, {path: 'url2'}] }
                    ttdUrls = json.data.map((item: any) => item.path || item.url).filter((url: string) => url);
                }

                if (ttdUrls.length > 0) {
                    const processedUrls = await Promise.all(
                        ttdUrls.map(async (url: string) => {
                            // Cek apakah URL sudah lengkap atau perlu base URL
                            const finalUrl = url.startsWith('http') ? url : `${API_BASE_URL}/${url.replace(/^\//, '')}`;
                            return makeImageTransparent(finalUrl);
                        })
                    );
                    setTtdHistory(processedUrls);

                    // JIKA HANYA ADA SATU TTD (Format API TUNGGAL), LANGSUNG APLIKASIKAN KE PREVIEW
                    if (ttdUrls.length === 1) {
