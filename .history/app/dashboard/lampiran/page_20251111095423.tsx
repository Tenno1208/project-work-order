...
  // ... (useEffect untuk /api/me dan /api/satker tetap sama) ...
  // ✅ Ambil data user
  useEffect(() => {
...
  };
  
  // ✅ HANDLER BARU untuk tanda tangan
  const handleTtdChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files || e.target.files.length === 0) return;
    const file = e.target.files[0];
    // Anda hanya bisa langsung membuat gambar transparan di sisi klien menggunakan
    // Canvas API. Untuk saat ini, kita hanya membuat URL preview.
    setTtdPelaporPreview(URL.createObjectURL(file));
  };
...
          }
          /* Aturan untuk mencetak tanda tangan */
          .ttd-area img {
            max-width: 150px;
            max-height: 80px;
            object-fit: contain;
            margin-bottom: -40px; /* Geser ke atas agar tidak jauh dari nama */
            z-index: 10;
             opacity: 0.8; /* Contoh efek */
          }
        }
        .big-box { border: 1px solid #000; min-height: 120px; padding: 8px; }
        .ttd-container { height: 100px; display: flex; justify-content: center; align-items: flex-end; }
        .ttd-container img { max-width: 100%; max-height: 100%; object-fit: contain; }
      `}</style>
...
        </div>
        
        {/* Tambahkan tombol upload tanda tangan di bagian non-print */}
        {!isPrintMode && (
          <div className="p-6 pt-0 no-print">
            <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold text-blue-600 hover:text-blue-700 transition">
              <Upload size={16} /> Upload Tanda Tangan Pelapor (PNG/JPG)
              <input
                type="file"
                accept=".jpg,.jpeg,.png"
...
        <div id="print-area" className="p-6">
          ...
          <div className="mt-4 flex gap-6">
            ...
            {/* ✅ Kepada Yth dari data satker.name */}
            ...
            <div className="col-span-9">
              {isPrintMode ? (
                ...
                <input
                  ...
                  // PERBAIKAN: Gunakan kelas Tailwind untuk input yang jelas
          ...
          <div className="mt-4 big-box text-sm">
            {isPrintMode ? (
              ...
              <textarea
                ...
                // PERBAIKAN: Tambahkan kelas input yang jelas pada textarea
...
              {/* ✅ LOGIKA TANDA TANGAN */}
              <div className={`mt-4 ttd-area ${ttdPelaporPreview ? 'ttd-container' : ''}`}>
                {ttdPelaporPreview && (
                  <img 
...
                )}
              </div>
              <div className="mt-4 text-sm"> {/* Atur margin agar nama pelapor berada di bawah TTD */}
                {form.pelapor || "(...........................)"}
              </div>
              <div className="text-xs mt-1">
