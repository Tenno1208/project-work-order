...
const API_BASE_URL = process.env.API_BASE_URL || "https://workorder123.loca.lt"; // Dari ENV template user
...
// TTD Proxy Lokal (Harus dibuat di server: app/api/ttd-proxy/[npp]/route.ts)
...
// --- HELPER FUNCTION TTD (MODIFIKASI: MENAMBAH TOKEN) ---
// Note: Hapus const `API_BASE_URL` dan `EXTERNAL_EDIT_BASE_URL` dari dalam fungsi
// karena sudah di-import di bagian atas file.

async function makeImageTransparent(imgUrl: string, token: string): Promise<string> {
    // Cek apakah URL ini sudah Data URL (Base64) atau masih URL biasa
    if (imgUrl.startsWith('data:')) {
        return imgUrl; // Jika sudah Base64, langsung kembalikan
    }

    return new Promise(async (resolve) => {
        try {
            // 1. FETCH GAMBAR DENGAN AUTHORIZATION HEADER
            const res = await fetch(imgUrl, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    // Menambahkan 'Accept' bisa membantu
                    'Accept': 'image/png, image/jpeg, image/gif',
                },
                // Penting untuk memastikan kredensial dikirim jika perlu
                // mode: 'cors',
            });

            if (!res.ok) {
                console.error(`Gagal fetch gambar TTD (${res.status}): ${imgUrl}`);
                return resolve(imgUrl); // Fallback ke URL asli jika gagal fetch
            }
            ...
            // 4. Proses Transparansi menggunakan Canvas (Logika Lama)
            const img = new Image();
            img.crossOrigin = "anonymous";
            
            // Masukkan data URL Base64 yang sudah diconvert ke img.src
            img.src = dataUrl;
...
            };
            img.onerror = () => resolve(imgUrl); // Fallback jika gagal load setelah konversi Base64
        } catch (error) {
            console.error("Error during TTD fetch/conversion:", error);
            resolve(imgUrl); // Fallback final
        }
    });
}
...
export default function LampiranPengajuanPage() {
    ...
    // #################################################
    // PENGAMBILAN RIWAYAT TTD
    // #################################################

   // ... (di dalam LampiranPengajuanPage function)
// #################################################
// PENGAMBILAN RIWAYAT TTD (Diubah)
// #################################################
const fetchTtdHistory = useCallback(async (token: string, npp: string) => {
    try {
        // 1. Memanggil endpoint proxy lokal TTD (Sekarang mengembalikan JSON)
        ...
        if (res.ok) {
            ...
            // Logika Adaptif: Cek format API (single ttd_url atau array data)
            if (json.ttd_url && typeof json.ttd_url === 'string') {
                ...
                // Ambil semua TTD yang ada di array 'data'
            ...
            if (ttdPaths.length > 0) {
                const processedUrls = await Promise.all(
                    ttdPaths.map(async (path: string) => {
                        ...
                        // 3. Konversi menjadi Data URL dengan transparansi
                        // Ini yang membuat gambar muncul dan bisa dipakai di modal/preview.
                        return makeImageTransparent(finalUrl); 
                    })
                );
    ...
    const fetchPengajuanDetail = useCallback(async (token: string, uuid: string, halOptions: HalOption[], satkers: SatkerDef[]) => {
        // Pindahkan setInitialLoading(true) ke sini untuk mode edit
        ...
        try {
            ...
            // Pindahkan setInitialLoading(false) ke sini untuk mode edit
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            // Pindahkan setInitialLoading(true) ke sini untuk mode non-edit (tambah baru)
            if (!storedUuid) {
                 setInitialLoading(true);
            }
            ...
            try {
                ...
                // 4. Fetch Riwayat TTD (Hanya jika NPP user tersedia)
                if (userNpp) {
                    await fetchTtdHistory(token, userNpp);
                }
                
                // 5. Jika mode edit, ambil data pengajuan detail
                if (storedUuid) {
                    await fetchPengajuanDetail(token, storedUuid, halOptionsMap, satkersMap);
                } else {
                    // Hanya setInitialLoading(false) jika bukan mode edit, 
                    // karena mode edit akan memanggilnya di dalam fetchPengajuanDetail
                    setInitialLoading(false);
                }
