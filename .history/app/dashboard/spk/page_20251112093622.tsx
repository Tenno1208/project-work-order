        ...
        <div className="bg-white rounded-3xl shadow-lg p-6 border border-blue-100">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="relative flex-1">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-black/60" size={20} />
              <input
                type="text"
                placeholder="Cari nomor SPK atau jenis pekerjaan..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full bg-gray-50 border-2 border-gray-300 rounded-2xl py-3.5 pl-12 pr-4 focus:border-cyan-500 focus:bg-white outline-none text-black transition-all"
              />
            </div>

            {/* Filter */}
            <div className="relative">
              <Filter className="absolute left-4 top-1/2 -translate-y-1/2 text-black/60" size={20} />
              <select
                value={filterStatus}
                onChange={(e) => setFilterStatus(e.target.value)}
                className="bg-gray-50 border-2 border-gray-300 rounded-2xl py-3.5 pl-12 pr-8 focus:border-cyan-500 focus:bg-white outline-none transition-all appearance-none cursor-pointer min-w-[180px] text-black"
              >
        ...
        <div className="bg-white rounded-3xl shadow-lg overflow-hidden border border-blue-100">
          {loading ? (
            <div className="py-20 text-center">
              <Loader2 className="mx-auto animate-spin text-cyan-600 mb-4" size={40} />
              <p className="text-black font-medium">Memuat data SPK...</p>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white">
                    ...
                    <th className="px-6 py-4 text-left font-semibold">Nomor SPK</th>
                ...
                <tbody>
                  {filteredSpk.map((spk, index) => (
                    <tr
                      key={spk.id}
                      className="border-b border-gray-200 hover:bg-gradient-to-r hover:from-cyan-50 hover:to-blue-50 transition-all cursor-pointer group"
                      onClick={() => router.push(`/dashboard/spk/${spk.id}`)}
                    >
                      <td className="px-6 py-4 text-black">{index + 1}</td>
                      <td className="px-6 py-4 font-semibold text-blue-700 group-hover:text-blue-800">{spk.nomor}</td>
                      <td className="px-6 py-4 text-black">{spk.pekerjaan}</td>
            ...
            <div className="py-20 text-center">
              ...
              <p className="text-black font-medium">Tidak ada data SPK ditemukan</p>
              <p className="text-black/70 text-sm mt-1">
