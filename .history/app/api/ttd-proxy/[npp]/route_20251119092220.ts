 ...
 * Proxy ini mengambil JSON dari API eksternal, mengekstrak URL TTD terbaru, 
 * dan mengembalikan DATA GAMBAR TTD tersebut untuk menghindari CORS.
...
  try {
    // 1. Ambil JSON dari API Eksternal (untuk mendapatkan URL TTD)
...
        console.error(`External TTD URL error (${ttdUrlResponse.status}):`, await ttdUrlResponse.text());
...
    let ttdPath = null;
    
    // Logika adaptif untuk mengambil path TTD terbaru (pertama di array data atau ttd_url)
...
      headers: {
        // Sertakan token jika API gambar PDAM membutuhkannya (biasanya tidak)
        // 'Authorization': token, 
      },
      cache: 'no-store',
    });
