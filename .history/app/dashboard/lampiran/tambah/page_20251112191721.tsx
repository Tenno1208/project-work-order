...
export default function LampiranPengajuanPage() {
  ...
  const handleAjukan = async () => {
    // Validasi field wajib (HANYA CEK hal/ID)
    const wajibDiisi = [
    ...
    // Penting: Hanya kirim ID Hal (form.hal) ke API
    Object.entries(form).forEach(([key, value]) => {
        // Kita hanya ingin mengirim form.hal (ID) ke API, bukan hal_nama
        if (key !== 'hal_nama') { 
        ...
        <div id="print-area" className="p-6">
          ...
          <div className="mt-4 flex gap-6">
            <div className="w-1/2 text-sm font-semibold">
              Hal:
              {isPrintMode ? (
                <span className="ml-2 font-normal">{form.hal_nama}</span> {/* Tampilkan NAMA */}
              ) : (
                <select
                  name="hal"
                  value={form.hal} // Bind ke ID
                  onChange={handleHalChange} // Gunakan handler baru
