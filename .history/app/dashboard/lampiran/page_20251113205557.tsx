...
const DELETE_API_BASE = "/api/pengajuan/{uuid}/delete"; // Updated: Gunakan API route internal
const MAX_RETRIES = 3; 
...
export default function DataPengajuanPage() {
    ...
    // UPDATED: Handler untuk konfirmasi delete menggunakan API route internal
    const handleConfirmDelete = async () => {
        ...
        // UPDATED: URL sekarang mengarah ke API route internal
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
            try {
                ...
                // UPDATED: Parse response JSON dari API route
                const responseData = await response.json();

                if (!response.ok) {
                    // Jika ada error dari API route
                    if (response.status === 401) {
                ...
                // Berhasil - hapus dari state lokal
                ...
                if (i === MAX_RETRIES - 1 || error.message.includes("Otorisasi Gagal")) {
                    const errorMsg = `Gagal menghapus data: ${error.message}`;
                    setAuthError(errorMsg);
                    showToast(errorMsg, 'error');
                    setIsDeleting(false);
                    setItemToDelete({id: null, hal: null});
