...
type ApiPengajuanDetail = {
    ...
    created_at: string;
    referensi_surat: string; // Tambah field referensi surat dari API
};
...
interface FormDataState {
    ...
    referensiSurat: string; // Tambah field referensi surat
}
...
export default function LampiranPengajuanPage() {
    ...
    const [form, setForm] = useState<FormDataState>({
        ...
        mengetahui: "", 
        nppMengetahui: "", 
        referensiSurat: "", // Inisialisasi field referensi surat
    });
    ...
    const fetchPengajuanDetail = useCallback(async (token: string, uuid: string, halOptions: HalOption[], satkers: SatkerDef[]) => {
        ...
        try {
            ...
            setForm({
                ...
                referensiSurat: data.referensi_surat || "", // Ambil data referensi surat dari API
            });
        ...
        <div className="p-6 min-h-screen">
            ...
            <div className="max-w-[900px] mx-auto bg-white border border-gray-300 shadow-md rounded-lg text-gray-800">
                ...
                <div id="print-area" className="p-6">
                    ...
                    {/* Field Referensi Surat dengan kotak dan teks yang lebih kecil */}
                    <div className="mt-3 flex items-center text-xs">
                        <div className="font-semibold mr-2">Referensi Surat:</div>
                        <div className="flex-1">
                            {isPrintMode ? (
                                <span>{form.referensiSurat || "-"}</span>
                            ) : (
                                <input
                                    type="text"
                                    name="referensiSurat"
                                    ...
                                    placeholder="Isi nomor referensi surat"
                                    className="w-1/2 p-0.5 border border-gray-300 rounded text-xs"
                                />
                            )}
                        </div>
                    </div>
                    <div className="text-xs text-gray-500 ml-[110px]">Opsional</div>
