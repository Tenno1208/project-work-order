  ...
  const filtered = pegawaiList
    .filter(
      (p) =>
        ...
        // Pastikan pegawai belum ditugaskan dengan membandingkan nama (case-insensitive)
        !assignedPeople.some((a) => a.name.toLowerCase() === p.name.toLowerCase())
    )
    .slice(0, 10);
...
const Chip = ({ person, onRemove, onTogglePic }) => (
    <div className="flex items-center bg-blue-100 text-blue-800 text-xs font-medium px-3 py-1 rounded-full mr-2 mb-2 shadow-sm border border-blue-200">
        ...
        {/* Show Nama (NPP) - Menampilkan nama lengkap dengan format yang konsisten */}
