...
export default function LampiranPengajuanPage() {
  ...
  const [satkerList, setSatkerList] = useState<SatkerDef[]>([]);
  ...
  // ✅ Fetch daftar satker dari API eksternal
  useEffect(() => {
    fetch("https://zephyrean-cycloidally-triston.ngrok-free.dev/api/satker_name")
      .then((res) => res.json())
      .then((data) => {
        // Pastikan data berupa array dan ubah jadi format {id,label}
        const satkers = (Array.isArray(data) ? data : []).map((item: any, i) => ({
          id: item.id?.toString() || `satker-${i}`,
          label: item.name || item.label || `Satker ${i + 1}`,
        }));
        setSatkerList(satkers);
      })
  ...
  const selectedSatker = satkerList.find((s) => s.id === form.satker);
        ...
        <div id="print-area" className="p-6">
          ...
          <div className="mt-10 flex justify-center text-center gap-24">
            <div>
              ...
              <div className="mt-14 text-sm">(...........................)</div>
