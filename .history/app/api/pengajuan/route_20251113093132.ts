...
export async function GET(req: Request) {
    try {
        ...
        // 🚨 PERBAIKAN NGrok/JSON: Baca sebagai teks dulu
        const rawText = await res.text();
        let json;
        try {
            json = JSON.parse(rawText);
             ...
             if (rawText.includes("ERR_NGROK_6024") || rawText.includes("ngrok")) {
                  ...
                  return NextResponse.json(
                      { success: false, message: "Akses API eksternal (Ngrok) diblokir. Harap periksa URL atau pastikan header Ngrok berfungsi. (ERR_NGROK_6024)" },
                      { status: 503 } 
                  );
             }
             // Tangkap jika API mengembalikan HTML/format tak terduga
