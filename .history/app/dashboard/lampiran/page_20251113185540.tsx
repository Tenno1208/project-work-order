...
export default function DataPengajuanPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        if (typeof window !== 'undefined') {
            // Menggunakan sessionStorage agar token tidak tersimpan permanen
    ...
    useEffect(() => {
        fetchData();
        // Hapus UUID edit yang tersimpan saat memuat halaman data
        if (typeof window !== 'undefined') {
    ...
    const handleBuatPengajuan = () => {
        ...
        if (typeof window !== 'undefined') {
            ...
            // Pastikan UUID edit dihapus saat membuat baru
            localStorage.removeItem('current_edit_uuid'); 
        }
    ...
    const handleEdit = (uuid: string) => {
        if (typeof window !== 'undefined') {
            // 1. Simpan UUID yang akan diedit ke localStorage
            localStorage.setItem('current_edit_uuid', uuid); 
            // Opsional: Simpan nomor surat sementara untuk tampilan loading
            localStorage.setItem("nomor_surat_terakhir", `Memuat data UUID ${uuid.substring(0,4)}...`);
        }
        // 2. Navigasi ke halaman form yang sama untuk mode edit
        router.push("/dashboard/lampiran/tambah"); 
    };
    // *** END PERUBAHAN ***
