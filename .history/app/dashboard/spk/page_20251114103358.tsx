...
const GET_API_SPK_URL = `${API_BASE_URL}/api/spk/lihat/data`;
const DELETE_API_SPK_URL = `${API_BASE_URL}/api/spk/delete`; // Asumsi API membutuhkan ID di body atau di path.
const MAX_RETRIES = 3;

// --- TYPES BARU SESUAI API ---
// Asumsi API mengembalikan field ini. Sesuaikan jika nama field berbeda (misal: tanggal_spk, status_pekerjaan)
type SPKItem = {
    id: number;
    nomor: string;
    pekerjaan: string;
    tanggal: string; // Tanggal dibuat/selesai
    status: "Selesai" | "Proses" | "Menunggu" | string;
    // Tambahkan field lain jika API mengembalikannya (misal: uuid)
    uuid?: string; 
};
...
export default function DaftarSPKPage() {
    ...
    const handleView = (spk: SPKItem) => {
        // Asumsi API membutuhkan ID atau UUID untuk melihat detail
        router.push(`/dashboard/spk/format?view=${spk.uuid || spk.id}`);
    };

    const handleEdit = (spk: SPKItem) => {
        // Asumsi API membutuhkan ID atau UUID untuk mengedit
        router.push(`/dashboard/spk/format?edit=${spk.uuid || spk.id}`);
    };
    ...
    const confirmDeletion = async () => {
        ...
        const spkId = modal.spkToDelete.id;
        closeModal();
        setLoading(true);
        ...
        try {
            // Menggunakan template URL DELETE_API_SPK_URL=${API_BASE_URL}/api/spk/{id}/delete
            const deleteUrl = `${DELETE_API_SPK_URL}/${spkId}/delete`; 
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.message || `Gagal menghapus SPK ${spkId}.`);
            }

            // Hapus dari state jika sukses
            setSpks(spks.filter((item) => item.id !== spkId));
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            for (let i = 0; i < MAX_RETRIES; i++) {
                try {
                    const res = await fetch(GET_API_SPK_URL, {
                        headers: {
                            "Authorization": `Bearer ${token}`,
                    ...
                    if (!res.ok || !result.success) {
                        throw new Error(result.message || `Gagal memuat data SPK. Status: ${res.status}`);
                    }
                    
                    // PENTING: Map data API ke format SPKItem
                    const mappedSpks: SPKItem[] = result.data.map((item: any) => ({
                        id: item.id,
                        nomor: item.nomor_spk || item.nomor,
                        pekerjaan: item.pekerjaan || item.nama_pekerjaan,
                        tanggal: item.tanggal_spk || item.tanggal,
                        status: item.status || "Menunggu", // Sesuaikan status default jika API tidak memberikan
            ...
            <div className="max-w-7xl mx-auto space-y-4">
                ...
                <div className="bg-white rounded-2xl shadow-lg p-5 border border-blue-100">
                    <div className="flex items-start justify-between mb-4 flex-wrap gap-4">
                        ...
                        <button
                            onClick={handleAdd}
                            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-teal-500 to-blue-600 text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all transform hover:scale-[1.02]"
                        >
                            <Plus size={20} />
                            Daftar SPK Baru
                        </button>
                    ...
                    {loading ? (
                        <div className="py-12 text-center">
                         ...
                         <div className="py-12 text-center bg-red-50/50">
                            ...
                            <p className="text-red-600/80 text-xs mt-1">Gagal memuat data dari {GET_API_SPK_URL}</p>
                        </div>
