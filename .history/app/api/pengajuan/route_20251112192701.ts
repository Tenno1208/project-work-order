...
export async function POST(req: Request) {
  try {
    ...
    // Field-field standar yang dikirim (Mapping eksplisit)
    const standardFields = [
      // Kunci Klien (Frontend) => Kunci Eksternal (API)
      { clientKey: "hal", externalName: "hal_id" }, 
      { clientKey: "kepada", externalName: "kepada" }, 
      { clientKey: "satker", externalName: "satker" }, 
      { clientKey: "kodeBarang", externalName: "kode_barang" }, // Mapped ke snake_case
      ...
      // Tambahkan field 'catatan' (Asumsi catatannya sama dengan keterangan jika tidak ada input terpisah)
    ...
    for (const field of standardFields) {
      ...
      // Jika field memiliki nilai (termasuk 0 atau string kosong yang valid)
      if (value !== null && value !== undefined) {
          
          let finalValue = String(value);

          // Khusus untuk 'hal' (ID dari frontend), kita petakan ke 'hal_id'
          if (field.clientKey === 'hal') {
              finalValue = String(value);
          }
          
          externalFormData.append(field.externalName, finalValue);
      } else {
        // Logging jika ada field wajib yang hilang, harusnya dicegah di frontend
        console.log(`[POST PENGAJUAN] Field kosong/hilang: ${field.clientKey} -> ${field.externalName}`);
      }
    }
    ...
    // === LOG UNTUK VERIFIKASI SEBELUM KIRIM KE NGROK ===
    console.log(`[POST PENGAJUAN] Mengirim data ke: ${EXTERNAL_API_URL}`);
    console.log(`[POST PENGAJUAN] Verifikasi Data Kritis:`);
    console.log(` - hal_id: ${externalFormData.get('hal_id')}`);
    console.log(` - kode_barang: ${externalFormData.get('kode_barang')}`);
    console.log(` - file lampiran count: ${fileIndex}`);
    // === END LOG ===
    ...
    const res = await fetch(EXTERNAL_API_URL, {
      ...
      headers: {
        ...
        // Hapus Content-Type: multipart/form-data; boundary=... agar fetch API node.js mengaturnya secara otomatis dan benar saat mengirim FormData
      },
