...
export default function SPKPage() {
  ...
  const [data, setData] = useState({
    ...
    uraianPekerjaan: "",
    status: "",
    jenisPekerjaan: "", // <-- SUDAH DITAMBAHKAN
  });
    ...
    const fetchJenisPekerjaan = async () => {
      try {
        ...
        if (json?.success && Array.isArray(json.data)) {
          // Mapping data ke format {id, nama}
          const mappedOptions = json.data.map((item: any) => ({
            id: item.id,
            nama: item.nama, // Menggunakan 'nama' yang sudah di-map di route.ts
          }));
          ...
          <div className="border border-black p-8 rounded-md bordered">
            ...
            <AnimatePresence>
              {showDetail && (
                ...
                {/* JENIS PEKERJAAN (Sudah Diperbaiki) */}
                <div className="flex mt-4 items-center">
                    ...
                    >
                        <option value="">
                          {jenisPekerjaanOptions.length > 0 ? "-- Pilih Jenis Pekerjaan --" : "Memuat atau Token Salah..."}
                        </option>
