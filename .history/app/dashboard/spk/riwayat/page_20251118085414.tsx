...
// --- KONSTANTA API (Sesuaikan dengan API Anda) ---
const GET_API_SPK_URL_LOCAL = "/api/spk/list"; // Ganti dengan endpoint GET daftar SPK Anda
const DELETE_API_SPK_URL_LOCAL = "/api/spk/delete/"; // Ganti dengan endpoint DELETE SPK Anda

const MAX_RETRIES = 3;

// --- TYPES SESUAI API (Disesuaikan untuk Riwayat/Daftar) ---
type SPKItem = {
    id: number; // ID internal database
    nomor: string; // Nomor SPK/UUID
    pekerjaan: string; // Jenis/Uraian Pekerjaan
    tanggal: string; // Tanggal SPK
    status: "Selesai" | "Proses" | "Menunggu" | string; // Status SPK
    uuid: string; // UUID untuk aksi edit/delete/view
    namaPetugas?: string; // Menambahkan field Petugas (opsional dari API)
    lokasi?: string; // Menambahkan field Lokasi (opsional dari API)
};
...
export default function RiwayatSPKPage() {
    ...
    // --- NAVIGASI Aksi (Disamakan dengan kode kedua) ---
    const handleAdd = () => router.push("/dashboard/spk/format"); // Ganti path sesuai rute Anda
    const handleView = (spk: SPKItem) =>
        router.push(`/dashboard/spk/format?view=${spk.uuid}`); // Ganti path sesuai rute Anda
    const handleEdit = (spk: SPKItem) =>
        router.push(`/dashboard/spk/format?edit=${spk.uuid}`); // Ganti path sesuai rute Anda
    ...
    const confirmDeletion = async () => {
        ...
        try {
            ...
            await new Promise((r) => setTimeout(r, 600)); // Animasi delay
    ...
    const fetchData = async () => {
        ...
        if (!token) {
            // Ini untuk skenario di mana token harus ada
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
            try {
                ...
                // Mapping data dari API ke format SPKItem
                const mapped: SPKItem[] = result.data.map((item: any) => ({
                    id: item.id || item.spk_id,
                    nomor: item.nomor_spk || item.uuid || item.id, // Ambil nomor dari field yang tersedia
                    pekerjaan: item.uraian_pekerjaan || item.jenis_pekerjaan || "Tidak ada data",
                    tanggal: item.tanggal_spk || item.tanggal || "-",
                    // Mapping status ke format yang diinginkan
                    status:
                        item.status === "pending"
                            ? "Menunggu"
                    ...
                    namaPetugas: item.nama_petugas || item.petugas || "-",
                    lokasi: item.lokasi_pekerjaan || item.lokasi || "-",
                }));

                setSpks(mapped.filter((i) => i.uuid)); // Hanya ambil data yang punya UUID/Nomor
                setLoading(false);
                return;
            } catch (err: any) {
                if (i === MAX_RETRIES - 1) {
    ...
    const filteredData = spks.filter((item) => {
        const matchSearch =
            ...
            item.namaPetugas?.toLowerCase().includes(searchQuery.toLowerCase()) || // Cari di Petugas
            item.pekerjaan.toLowerCase().includes(searchQuery.toLowerCase());

        const statusToCheck = item.status === "Dalam Proses" ? "Proses" : item.status; // Penyesuaian: Proses/Dalam Proses
    ...
    const statusCount = {
        ...
        menunggu: spks.filter((d) => d.status === "Menunggu" || d.status === "Pending").length, // Menunggu/Pending dianggap sama
    };
    ...
    const getStatusColor = (status: string, isForCard = false) => {
        ...
        if (isForCard) {
            switch (normalizedStatus) {
                case "Selesai":
                    return "bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-4 border border-green-200";
                case "Proses":
                    return "bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-4 border border-yellow-200";
                case "Menunggu":
                    return "bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200";
                case "Total": // Untuk Total SPK di summary
                    return "bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-4 border border-purple-200";
                default:
                    return "bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg p-4 border border-gray-200";
            }
        }
        ...
        <div className="space-y-4 p-4 md:p-6 bg-gray-50 min-h-screen">
            ...
            <div className="bg-white rounded-xl shadow-md p-5 border border-blue-100">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-black">Riwayat SPK</h2>
                        <p className="text-black text-sm">
                            Pantau dan kelola riwayat Surat Perintah Kerja
                        </p>
                    </div>
                </div>
                ...
                <div className="flex gap-3 flex-wrap items-center">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button
                            onClick={handleAdd}
                            className="p-2.5 flex items-center gap-1 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-sm"
                            title="Tambah SPK Baru"
                        >
                            <Plus size={16} />
                            <span className="hidden md:inline">Tambah SPK</span>
                        </button>
                    </div>
                </div>
            </div>
                                ...
                                <tbody className="divide-y divide-black/10">
                                    {filteredData.length === 0 ? (
                                        ...
                                        filteredData.map((item, index) => (
                                            <tr key={item.uuid} className="hover:bg-blue-50 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-xs text-black whitespace-nowrap">{item.lokasi}</td>
