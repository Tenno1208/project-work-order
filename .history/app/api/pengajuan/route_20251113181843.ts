...
// URL baru yang spesifik untuk GET (melihat)
const GET_API_URL = "https://brave-chefs-refuse.loca.lt/api/pengajuan/lihat"; 
...
export async function GET(req: Request) {
    try {
        ...
        // Meneruskan request ke API eksternal menggunakan URL GET yang baru
        const res = await fetch(GET_API_URL, { // <-- Perubahan di sini
            method: 'GET',
            headers: {
                Accept: "application/json",
                'Authorization': `Bearer ${token}`,
                // Hapus ngrok-skip-browser-warning karena URL-nya sekarang loca.lt
        ...
        try {
            json = JSON.parse(rawText);
             ...
             // Hapus pemeriksaan ngrok karena URL-nya sekarang loca.lt (kecuali Anda ingin menambahkan penanganan loca.lt)
...
export async function POST(req: Request) {
    try {
        ...
        for (const field of standardFields) {
            ...
            if (value) {
                // Konversi 'hal' (hal_id) ke number jika perlu, lainnya ke string
