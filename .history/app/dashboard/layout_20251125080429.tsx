...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    /**
     * Mengambil permissions dari API dan menyimpannya ke localStorage.
     * Logika parsing data permissions lebih dijamin (robust).
     */
    const fetchAndStorePermissions = async (token: string) => {
        try {
            ...
            // Logika parsing: pastikan mengambil array dari field 'data'
            const rawPermissions = Array.isArray(json.data) ? json.data : [];
            
            const permissions: string[] = rawPermissions.map((p: any) => {
                // Ambil field 'name' jika ada, atau gunakan objek itu sendiri jika itu adalah string.
                return (p && p.name) ? p.name.toString() : (typeof p === 'string' ? p : '');
            }).filter(Boolean); // Hapus nilai kosong/undefined

            // LOGGING UNTUK DIAGNOSIS
            console.log("Permissions berhasil di-parse:", permissions); 

            localStorage.setItem("user_permissions", JSON.stringify(permissions));
            console.log("✅ User Permissions loaded from API and stored in localStorage.");
            return permissions;
        } catch (err) {
    ...
    useEffect(() => {
        ...
        const loadInitialData = async () => {
            ...
            // 3. Jika Data User TIDAK ADA (atau gagal parse), Panggil API
            ...
            // 4. Jika Permissions TIDAK ADA (atau gagal parse), Panggil API
            // (Menggunakan hasil dari fetchAndStorePermissions yang sekarang lebih robust)
