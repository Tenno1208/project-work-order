...
const SUPERVISOR_API_PATH = "/api/proxy?url=" + encodeURIComponent("https://gateway.pdamkotasmg.co.id/api-gw/portal-pegawai/api/auth/my-supervisor"); // Menggunakan proxy
...
// --- MODAL BARU UNTUK RIWAYAT TTD (OMITTED) ---
const TtdHistoryModal = (props: any) => { /* implementation */ return props.isOpen ? (<div>TtdHistoryModal</div>) : null; };
const TtdCropModal = (props: any) => { /* implementation */ return props.isOpen ? (<div>TtdCropModal</div>) : null; };

// --- HELPER FUNCTION TTD (OMITTED) ---
...
export default function LampiranPengajuanPage() {
    ...
    // NEW STATE: Menyimpan nomor telepon supervisor
    ...
    // REUSED FUNCTION: fetchTtdHistory
    ...
    const fetchSupervisorData = useCallback(async (token: string) => {
        try {
            // Menggunakan proxy API lokal karena SUPERVISOR_API_PATH adalah URL eksternal
            const res = await fetch(SUPERVISOR_API_PATH, {
                headers: { Authorization: `Bearer ${token}` },
                cache: "no-store",
            });
            ...
            if (res.ok) {
                ...
                if (supervisorData?.name && supervisorData?.npp) {
                    ...
                    // 2. Update hidden contact (save to state/localStorage)
            ...
            // Hanya tampilkan error jika tidak di mode edit dan form 'mengetahui' masih kosong
            if (!isEditMode && form.mengetahui === "") {
                setNotification({ type: 'error', message: 'Gagal memuat data supervisor. Akan dicoba dari data pegawai.' });
            }
        }
    }, [setNotification, isEditMode, form.mengetahui]);


    // REUSED FUNCTION: fetchPengajuanDetail
    ...
    useEffect(() => {
        // Logika ini dipanggil saat form.satker berubah (seharusnya hanya terjadi saat load dari LS, atau mode edit)
        // ABAIKAN jika data supervisor sudah diisi dari API spesifik (di fetchSupervisorData)
        if (form.mengetahui) return;
        ...
        // Mencoba mencari pegawai berdasarkan ID dari API atau mencocokkan label Satker
    ...
    const proceedSubmission = useCallback(async () => {
        ...
        try {
            ...
            // NEW: Ambil TLP supervisor dari localStorage dan sertakan dalam payload
            ...
            for (let i = 0; i < MAX_RETRIES; i++) {
                try {
                    ...
                    if (finalResult.success) {
                        ...
                        localStorage.removeItem('current_edit_uuid');
                        localStorage.removeItem('supervisor_tlp'); // Bersihkan TLP setelah sukses
                        router.push("/dashboard/lampiran");
                        return;
                    }
    ...
    const handleAjukan = async () => {
        const wajibDiisi = [
            ...
            // Menambahkan validasi untuk Supervisor
            { field: "mengetahui", label: "Nama Mengetahui" },
            { field: "nppMengetahui", label: "NPP Mengetahui" },
        ];
