...
<div className="mt-4 flex gap-20">
    <div className="w-1/2 text-sm font-semibold">
        ...
        {/* --- TAMBAHAN: Ref. Surat di bawah Hal persis --- */}
        <div className="mt-2 text-xs font-semibold"> {/* Ubah text-sm menjadi text-xs */}
            Ref. Surat :
            {isPrintMode ? (
                <span className="ml-2 font-normal text-xs">{form.referensiSurat || "-"}</span> {/* Tambah text-xs */}
            ) : (
                <Select
                    name="referensiSurat"
                    ...
                    className="ml-2 w-40 text-xs select-input-only" {/* Ubah w-3/4 menjadi w-40 (pendek), text-sm menjadi text-xs */}
                    styles={{
                        menu: (base) => ({ ...base, zIndex: 50, position: "absolute" }),
                        control: (base) => ({ ...base, fontSize: '0.75rem', border: '1px solid #d1d5db', borderRadius: '0.25rem', background: 'white' }), {/* Tambah fontSize kecil */}
                    }}
                    menuPlacement="auto"
                    isClearable
                />
            )}
            <div className="text-xs text-gray-500 italic mt-1 ml-2"> {/* Sudah text-xs */}
