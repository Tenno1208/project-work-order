...
export default function DaftarSPKPage() {
  ...
  const showNavigationModal = (action: string, spkNumber?: string) => {
    const message = `Anda akan diarahkan ke halaman ${action} di: ${TARGET_PATH}`;
    setModal({
      isOpen: true,
      message,
      path: TARGET_PATH,
        ...
        <div className={`bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm transform transition-all ${modal.isDeletion ? 'border-t-4 border-red-500' : 'border-t-4 border-blue-500'}`}>
          ...
          {modal.isDeletion ? (
            ...
            <div className="flex justify-end">
              ...
              >
                {/* Biasanya ini akan menjadi router.push(modal.path) */}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center whitespace-nowrap">
                        <div className="flex justify-center gap-2">
                          ...
                          {/* 3. Tombol Hapus (Trash2) -> Menggunakan Custom Modal */}
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDelete(spk);
