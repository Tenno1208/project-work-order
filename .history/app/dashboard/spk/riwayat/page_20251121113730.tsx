...
const GET_API_SPK_URL_LOCAL = "/api/spk/riwayat/{npp}"; 
    ...
    const fetchData = async () => {
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
            try {
                ...
                // Logika Sederhana: Status 404 atau 401 dianggap error dan menampilkan pesan khusus.
                if (!res.ok) {
                    let errorMessage = `Gagal memuat data (Status ${res.status})`;
                    if (res.status === 404 || res.status === 401) {
                        // Pesan khusus untuk kasus 404/401
                        errorMessage = "Anda belum memiliki riwayat pengajuan SPK."; 
                    }
                ...
                // Hanya set data yang valid jika sukses dan ada data, jika kosong, set array kosong.
                ...
                {loading ? (
                    ...
                    <div className="py-12 text-center bg-red-50/50">
                        ...
                        <p className="text-red-700 text-sm font-medium">
                            {/* Menampilkan pesan yang spesifik jika 404/401 */}
                            {error.includes("belum memiliki riwayat") ? "Anda belum memiliki riwayat pengajuan." : `Error: ${error}`}
                        </p>
                        <p className="text-red-600/80 text-xs mt-1">
                            {error.includes("belum memiliki riwayat") ? "Silakan buat pengajuan SPK baru." : "Silakan periksa konfigurasi token dan API server."}
                        </p>
                    </div>
                ) : (
                            ...
                            <table className="min-w-full">
                                ...
                                <tbody className="divide-y divide-black/10">
                                    {filteredData.length === 0 ? (
                                        // --- Pesan Kosong (Berlaku untuk Data Utama Kosong ATAU Search Kosong) ---
                                        <tr>
                                            <td colSpan={7} className="px-6 py-10 text-center">
                                                <div className="flex flex-col items-center gap-2">
                                                    {/* Ikon Search/Drop/History tergantung apakah ada searchQuery */}
                                                    ...
                                                    <p className="text-sm text-black font-medium">
                                                        {searchQuery.length > 0 ? "Tidak ada data yang ditemukan" : "Anda belum memiliki riwayat pengajuan."}
                                                    </p>
                                                    <p className="text-xs text-black/60">
                                                        {searchQuery.length > 0 ? "Coba ubah filter atau kata kunci pencarian" : "Silakan buat pengajuan baru."}
                                                    </p>
                                                </div>
                                            </td>
                                        </tr>
