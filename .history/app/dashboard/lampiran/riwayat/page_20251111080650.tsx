      ...
      >
        ...
        <nav className="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            // Active hanya jika pathname sama persis dengan item.href
            const active = pathname === item.href;
            const isExpanded = expandedItems.includes(item.href);
            return (
              <div key={item.href}>
                <div
                  className={`group flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 w-full relative overflow-hidden cursor-pointer ${
                    active
