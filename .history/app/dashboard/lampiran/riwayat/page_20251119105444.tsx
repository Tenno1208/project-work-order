...
// ==========================================================
// PENGGUNAAN ENVIRONMENT VARIABLES UNTUK ENDPOINT API
// Pastikan Anda telah mendefinisikan API_BASE_URL di .env Anda
// ==========================================================
const API_BASE_URL = process.env.API_BASE_URL || "https://workorder123.loca.lt";

// Endpoint Next.js API route untuk mengambil data pengguna (NPP)
const USER_API_PATH = "/api/me"; 
// Endpoint Riwayat Pengajuan yang difilter berdasarkan NPP Pelapor
const LIST_API_PATH = `${API_BASE_URL}/api/pengajuan/pelapor`; // Akan ditambahkan /{npp} di fetchData
...
        };
        ...
        // --- LANGKAH 1: Dapatkan NPP Pengguna yang Sedang Login ---
        let npp: string | null = null;
        try {
            const userRes = await fetch(USER_API_PATH, { headers });
            const userData = await userRes.json();
            if (userData?.npp) {
                npp = userData.npp;
            } else {
                showToast('Gagal mengidentifikasi NPP pengguna.', 'error');
                throw new Error("Gagal mendapatkan NPP pengguna.");
            }
        } catch (error) {
            console.error("Gagal fetch data pengguna:", error);
            setAuthError("Gagal mengidentifikasi pengguna. Mohon login ulang.");
            setLoading(false);
            return;
        }

        // --- LANGKAH 2: Buat URL API yang difilter dengan NPP ---
        const finalApiUrl = `${LIST_API_PATH}/${npp}`;
        console.log(`Fetching data dari: ${finalApiUrl}`);


        for (let i = 0; i < MAX_RETRIES; i++) {
            try {
                const response = await fetch(finalApiUrl, { // Gunakan URL yang sudah difilter
                    method: 'GET',
                    headers: headers,
                }); 
                    
...
            // Pastikan UUID edit lama dihapus
...
        router.push(`/dashboard/lampiran/tambah`); // Pindah ke /tambah untuk logika edit di sana
    };
...
        const deleteUrl = `${DELETE_API_BASE}${uuidToDelete}`; // Menggunakan template literal yang lebih aman
