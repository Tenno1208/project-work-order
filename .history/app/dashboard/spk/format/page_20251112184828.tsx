...
export default function SPKPage() {
  ...
  cconst [jenisPekerjaanOptions, setJenisPekerjaanOptions] = useState<string[]>([]);
...
useEffect(() => {
    const fetchJenisPekerjaan = async () => {
      try {
        ...
        if (!res.ok) throw new Error("Gagal memuat data Jenis Pekerjaan"); // GANTI PESAN
        const json = await res.json();

        if (json?.success && Array.isArray(json.data)) {
          // GANTI FIELD: dari 'nama_jenis' -> 'nama_pekerjaan'
          const names = json.data.map((item: any) => item.nama_pekerjaan); 
          setJenisPekerjaanOptions(names); // GANTI STATE SETTER
        } else {
          console.error("Format data Jenis Pekerjaan tidak sesuai:", json); // GANTI PESAN
        }
      } catch (err) {
        console.error("❌ Error ambil data Jenis Pekerjaan:", err); // GANTI PESAN
      }
    };
