...
// Import ikon baru: Pencil (Edit) dan Trash2 (Delete)
import { History, Eye, Download, Search, Pencil, Trash2 } from "lucide-react";
...
export default function RiwayatDataPengajuan() {
  ...
  // --- Fungsi Aksi Baru ---
  const handleView = (item: RiwayatItem) => {
    alert(`Melihat detail ID: ${item.id} - ${item.namaPengaju}`);
    // Tambahkan logika navigasi atau modal untuk detail
  };

  const handleEdit = (item: RiwayatItem) => {
    alert(`Mengedit data ID: ${item.id} - ${item.namaPengaju}`);
    // Tambahkan logika navigasi ke halaman edit atau modal form
  };

  const handleDelete = (item: RiwayatItem) => {
    if (confirm(`Apakah Anda yakin ingin menghapus pengajuan ID: ${item.id}?`)) {
      alert(`Menghapus data ID: ${item.id}`);
      // Tambahkan logika untuk menghapus data dari state/API
    }
  };

  const handleDownload = (item: RiwayatItem) => {
    alert(`Mengunduh dokumen untuk ID: ${item.id}`);
                  ...
                  <td className="px-4 py-3 text-xs font-medium whitespace-nowrap">
                    <div className="flex gap-1.5">
                      ...
                      <button
                        ...
                        className="p-1.5 bg-yellow-100 hover:bg-yellow-200 rounded-md transition-colors duration-200"
                        title="Edit Pengajuan"
                      >
                        <Pencil
                          className="text-yellow-700 hover:scale-110 transition-transform"
                          size={14}
                        />
                      </button>
                      ...
                      <button
                        ...
                        className="p-1.5 bg-red-100 hover:bg-red-200 rounded-md transition-colors duration-200"
                      ...
                      {/* Tombol Download Dokumen (sudah ada) */}
                      <button
        ...
        <div className="flex items-center justify-between">
          <div className="text-xs text-black">
            ...
            <ul className="mt-1 space-y-0.5 list-disc list-inside">
              ...
              <li>Klik ikon pensil untuk mengedit pengajuan</li>
              <li>Klik ikon tempat sampah untuk menghapus pengajuan</li>
