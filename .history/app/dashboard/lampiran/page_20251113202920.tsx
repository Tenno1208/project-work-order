...
// MENGGUNAKAN ENV BARU SESUAI PERMINTAAN USER: DELETE_API_PENGAJUAN_URL
const DELETE_API_BASE = process.env.DELETE_API_PENGAJUAN_URL || "http://default-delete-api-gagal-dimuat.com";
const MAX_RETRIES = 3; 
...
        
        if (typeof window !== 'undefined') {
            // Menggunakan localStorage untuk token
...
                    setLoading(false);
                    // Jika gagal total, tampilkan pesan error khusus untuk pengguna
                    if (i === MAX_RETRIES - 1 && !error.message.includes("Otorisasi Gagal")) {
                        showToast(`Gagal total memuat data: ${error.message.substring(0, 50)}...`, 'error');
                    }
                    return; 
                }
                
...
    useEffect(() => {
        fetchData();
    }, [fetchData]); // Dependency hanya pada fetchData yang merupakan useCallback
...
        setTimeout(() => {
            setCreating(false);
            // Navigasi ke halaman tambah (SIMULASI ROUTING)
            // PATH DIPERBAIKI: dashboard/lampiran/tambah/page.tsx
            router.push("/dashboard/lampiran/tambah/page.tsx"); 
        }, 500);
    };

    const handleView = (uuid: string) => {
        // Navigasi ke halaman detail (SIMULASI ROUTING)
        router.push(`/dashboard/lampiran/${uuid}`); 
    };
    
    const handleEdit = (uuid: string) => {
        // Navigasi ke halaman edit (SIMULASI ROUTING)
        router.push(`/dashboard/lampiran/edit/${uuid}`);
    };

    const handleDeleteClick = (id: number, hal: string) => {
        if (isDeleting) return; // Mencegah aksi jika sedang menghapus
...
        const headers: HeadersInit = {
            'Authorization': `Bearer ${token}`,
            // Header untuk bypass localtunnel jika API menggunakan localtunnel
            'bypass-tunnel-reminder': 'true' 
        };

        // URL DELETE diambil dari ENV, mengganti {id} dengan ID yang sebenarnya
        const deleteUrl = `${DELETE_API_BASE.replace('{id}', idToDelete.toString())}`;
...
                        throw new Error(`Otorisasi Gagal: Status ${response.status}. Token bermasalah atau tunnel memblokir.`);
                    }
                    throw new Error(`Gagal menghapus. Status: ${response.status}. Pesan: ${errorBody.message || response.statusText}`);
                }
                
...
                    const errorMsg = `Gagal total menghapus data: ${error.message.substring(0, 100)}...`;
                    setAuthError(errorMsg);
                    showToast(errorMsg, 'error');
                    setIsDeleting(false);
                    setItemToDelete({id: null, hal: null});
...
            <Toast toast={toast} setToast={setToast} /> {/* Gunakan komponen Toast */}
...
                                            {/* Action: DELETE (Hapus Data) - Menggunakan ID */}
                                            <button
