...
async function uploadFile(file: File, token: string, customName: string, type: 'ttd' | 'pengajuans'): Promise<string> {
    ...
    // Tampilkan respons lengkap di log server Next.js jika gagal mendapatkan URL
    if (!res.ok || !result.data?.fileUrl) {
         console.error(`❌ Respons Gagal File Handler (${type}): HTTP ${res.status}. Body:`, result);
    }

    // Cek sukses berdasarkan Status HTTP dan adanya fileUrl (berdasarkan Postman)
    if (res.ok && result.data?.fileUrl) { 
        return result.data.fileUrl; 
    } else { 
        // 1. Ambil pesan API yang paling spesifik (message atau status dari body)
        const apiMessage = result.message || result.status || 'Tidak ada pesan spesifik dari File Handler.';
        
        // 2. Buat pesan error yang akan dilempar
        let detailedErrorMsg = `API PDAM merespons HTTP ${res.status}. Pesan dari body: ${apiMessage}`;

        // Jika status HTTP 200 (res.ok = true) tapi gagal mendapatkan URL
        if (res.status === 200) {
             detailedErrorMsg = `Upload Gagal (Status 200 OK). Pesan API: ${apiMessage}`;
        }

        // Lempar error ke fungsi POST utama
        throw new Error(`Gagal upload File Handler (${type}). ${detailedErrorMsg}`);
    }
}
...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (ttdFile instanceof File && ttdFile.size > 0) {
            ...
            // Jika Anda ingin menggunakan URL TTD yang sudah ada (dari mode edit), Anda harus
            // mengirimkannya dari frontend, bukan melalui ttdFile di formData.
            // Untuk sementara, ttdUrl dibiarkan undefined, yang berarti API Pengajuan Anda
            // mungkin menolak karena TTD wajib diisi.
        } else {
             // Jika ada file TTD lama (existing TTD URL) yang dikirim dari frontend,
             // Anda bisa mengambilnya di sini dan menetapkannya ke ttdUrl.
             // (Asumsi saat ini Anda hanya mengandalkan ttdFile baru atau tidak ada TTD).
        }
        // ==========================================================
        // >> END MODIFIKASI: MELEWATI UPLOAD TTD SESUAI PERMINTAAN <<
        // ==========================================================

        // Proses Upload File Lampiran (Tidak dilewati)
        let fileIndex = 0;
        while (fileIndex < 4) { 
