...
export default function LampiranPengajuanPage() {
  ...
  const [satkers, setSatkers] = useState<SatkerDef[]>([]); // ✅ satker dari API
...
    .then((res) => res.json())
    .then((data) => {
      // ✅ PERBAIKAN 1: Pastikan data.data adalah array dan ada
      if (!Array.isArray(data?.data)) {
        console.error("❌ Struktur data tidak sesuai atau data kosong:", data);
        return;
      }
      
      // ✅ PERBAIKAN 2: Mapping menggunakan properti 'id' dan 'satker_name'
      const mapped = data.data.map((item: any) => ({
        id: item.id?.toString(), // Gunakan 'id' sebagai value dropdown
        label: item.satker_name, // Gunakan 'satker_name' sebagai label dropdown
      }));

      // Optional: Log untuk debugging
      console.log("✅ Data Satker berhasil dimuat. Jumlah:", mapped.length); 
      console.log("Contoh Satker:", mapped[0]);
...
    .catch((err) => console.error("Gagal ambil satker:", err));
  ...
  const jabatan = selectedSatker ? getJabatanFromSatker(selectedSatker.label) : "Ka.Unit";
