// File: LampiranPengajuanPage.tsx (Kode final yang sudah disesuaikan)
...
// NOTE: Komponen Notification, ConfirmationModal, TtdHistoryModal, TtdCropModal dihilangkan untuk brevity
// NOTE: Fungsi getCroppedImg, createImage, getRadianAngle, processImageTransparency dihilangkan untuk brevity

// --- HELPER FUNCTION TTD (FINAL UNTUK MENGATASI CORS) ---
// Note: Kode Anda untuk makeImageTransparent di bawah ini ASUMSI menggunakan kode
// Cropper/Canvas yang sudah Anda definisikan sebelumnya (dihilangkan untuk brevity)

// ASUMSI fungsi yang diperlukan untuk Cropper/Canvas (createImage, processImageTransparency, etc.)
// SUDAH didefinisikan di atas file ini, seperti yang Anda tunjukkan di prompt sebelumnya.
...
// Asumsi fungsi getCroppedImg dan processImageTransparency sudah didefinisikan


// 🎯 FUNGSI UTAMA DENGAN PERBAIKAN CORS
async function makeImageTransparent(imgUrl: string, token: string, settings?: { whiteThreshold?: number, blackThreshold?: number, useAdvanced?: boolean }): Promise<string> {
    if (imgUrl.startsWith('data:')) {
        // Jika sudah Data URL (Base64), langsung proses transparansi
    ...
    return new Promise(async (resolve) => {
        try {
            ...
            // 🎯 PERBAIKAN UTAMA: Bungkus URL eksternal ke dalam proxy lokal
            if (imgUrl.startsWith('http')) {
                const encodedExternalUrl = encodeURIComponent(imgUrl);
                // Menggunakan /api/file-proxy yang Anda definisikan
                fetchUrl = `/api/file-proxy?url=${encodedExternalUrl}`; 
                console.log(`➡️ Menggunakan File Proxy untuk: ${imgUrl}`);
            } else {
                // Untuk path relatif atau Base64 yang lolos di awal
                fetchUrl = imgUrl; 
            }
...
// Note: processImageTransparency dan Cropper helpers (seperti createImage) di sini hanya placeholder
// karena fungsinya sudah Anda berikan sebelumnya, dan diasumsikan berada di scope file.
// --- PLACEHOLDER START ---
async function processImageTransparency(dataUrl: string, settings?: any): Promise<string> {
    // Logic Anda untuk memproses Canvas dan transparansi
    return dataUrl; 
}
// --- PLACEHOLDER END ---
            ...
            if (uniqueTtdPaths.length > 0) {
                const processedUrls = await Promise.all(
                    uniqueTtdPaths.map(async (path: string) => {
                        ...
                        // 🎯 makeImageTransparent sekarang menggunakan /api/file-proxy untuk mengatasi CORS
                ...
                setTtdPelaporPreview(null); // Pastikan preview bersih jika tidak ada TTD
            }
        } else {
                            ...
                            <div className={`mt-3 text-center no-print ${ttdPelaporPreview ? 'mt-2' : ''}`}>
                                {!isPrintMode && (
                                    <button
                                        ...
                                        // Styling button disesuaikan agar tidak menjadi tombol besar jika TTD sudah ada
                                        className={`flex items-center justify-center mx-auto gap-2 cursor-pointer transition 
                                            ${ttdPelaporPreview 
                                                ? 'text-red-600 hover:text-red-800 text-xs underline font-medium' 
