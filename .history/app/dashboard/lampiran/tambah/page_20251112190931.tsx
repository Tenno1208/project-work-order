...
export default function LampiranPengajuanPage() {
  ...
  const [form, setForm] = useState({
    ...
    pelapor: "",
    nppPelapor: "",
    mengetahui: "", // Field ini belum terisi otomatis, perlu diisi manual
    nppMengetahui: "", // Field ini belum terisi otomatis, perlu diisi manual
  });
  ...
  const handleAjukan = async () => {
    ...
    files.forEach((file, index) => {
      // Menggunakan nama field 'file0', 'file1', dst. untuk FormData
      formData.append(`file${index}`, file, file.name); 
    });

    // Tambahkan tanda tangan
    if (ttdPelaporFile) {
      // Menggunakan nama field 'ttdPelapor'
    ...
    try {
      const res = await fetch("/api/pengajuan", {
        method: "POST",
        body: formData, // Browser akan otomatis mengatur Content-Type: multipart/form-data
      });
      ...
      if (result.success) {
        ...
        // Jika sukses, mungkin diarahkan ke halaman riwayat atau clear form
            ...
            <div>
              ...
              <div className="mt-1">
                {isPrintMode ? (
                  <div className="h-[70px] flex items-center justify-center">
                    {/* Kotak kosong di print mode */}
                  </div>
                ) : (
                  // Input manual untuk Mengetahui (di luar print mode)
                  <div className="mt-2">
                    <input
                      type="text"
                      name="mengetahui"
                      ...
                      className="text-center p-1 border border-gray-300 rounded text-sm w-full"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="text-xs mt-1">
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="NPP"
                  className="text-center p-1 border border-gray-300 rounded text-xs w-28"
                />}
              </div>
            </div>
