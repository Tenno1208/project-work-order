...
if (data.ttd_pelapor) {
    ...
    useEffect(() => {
    const satkerId = form.satker; // Ini adalah ID Satker (string)
    const selectedSatker = satkers.find(s => s.id === satkerId);

    if (satkerId && selectedSatker) {
        // Asumsi: Kepala Unit/Jabatan tertinggi adalah pegawai dengan jabatan tertentu (misal 'Ka.Unit' atau 'Kepala')
        const targetJabatan = selectedSatker.jabatan; // Ambil jabatan yang dicari dari data satker

        // Cari pegawai yang satker_id-nya cocok DAN jabatannya sesuai target
        const penanggungJawab = allPegawai.find(peg => 
            String(peg.satker_id) === satkerId && 
            peg.jabatan.toLowerCase().includes(targetJabatan.toLowerCase())
        );
        ...
        if (penanggungJawab) {
            ...
            // Fallback: Cari pegawai pertama di satker itu
            const fallbackPegawai = allPegawai.find(peg => String(peg.satker_id) === satkerId);
            setForm(prev => ({
                ...prev,
                mengetahui: fallbackPegawai?.name || "",
                nppMengetahui: fallbackPegawai?.npp || "",
            }));
            if (!fallbackPegawai) {
                setNotification({ type: 'warning', message: `Tidak ditemukan Penanggung Jawab (${targetJabatan}) di Satker ini. Harap isi manual.` });
            }
        }
    } else if (!satkerId) {
        // Kosongkan jika Satker di-reset
