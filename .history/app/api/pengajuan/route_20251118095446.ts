...
export async function POST(req: Request) {
    ...
    try {
        ...
        // --- LOGIKA SANITASI FALLBACK DIHAPUS/DIKONTROL KETAT ---
        if (responseJson?.data) {
             // Membersihkan TTD Pelapor (jika ada)
            if (typeof responseJson.data.ttd_pelapor === 'string') {
                responseJson.data.ttd_pelapor = responseJson.data.ttd_pelapor.replace(/\\\//g, '/');
            }

            // TIDAK PERLU SANITASI responseJson.data.file KARENA LARAVEL SUDAH MENGGUNAKAN CASTING
            // KITA HANYA PERLU MEMASTIKAN VALUE-NYA TIDAK null/undefined/non-string sebelum diakses.
        }
