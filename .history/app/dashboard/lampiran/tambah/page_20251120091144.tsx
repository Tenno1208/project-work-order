...
// --- MODAL RIWAYAT TTD (TIDAK BERUBAH) ---
...
// --- MODAL BARU UNTUK CROPPING TTD ---
const TtdCropperModal = ({ 
    isOpen, 
    imageUrl, 
    originalFile,
    onCropComplete, 
    ...
    // State lokal untuk simulasi cropping
    const [scale, setScale] = useState(1);
    const [rotation, setRotation] = useState(0);

    if (!isOpen || !imageUrl || !originalFile) return null;

    // FUNGSI SIMULASI CROPPING & TRANSPARANSI (MENGGANTIKAN LIBRARY CROPPING)
    const handleSimulatedCrop = async () => {
        // Dalam implementasi nyata, di sini Anda akan menggunakan getCroppedImage dari react-easy-crop
        // atau canvas.getCroppedCanvas().toBlob() dari react-cropper.

        // Karena tidak ada library, kita akan membuat image yang baru (hanya mengubah skala & rotasi)
        // dan menjalankan fungsi transparansi Anda yang sudah ada, lalu mengubahnya kembali menjadi File.
        ...
        tempImg.onload = async () => {
            ...
            const ratio = 1.5; // Rasio tinggi/lebar tanda tangan yang ideal

            // Set ukuran canvas agar proporsional untuk TTD (misal 300x200)
            canvas.width = 300;
            canvas.height = 300 / ratio;

            // Hitung ukuran dan posisi gambar di canvas agar bisa di-"zoom"
            const imgWidth = tempImg.width * scale;
            const imgHeight = tempImg.height * scale;
            
            ctx.clearRect(0, 0, canvas.width, canvas.height); // Bersihkan canvas

            // Lakukan Rotasi (Simulasi)
            ctx.translate(canvas.width / 2, canvas.height / 2);
            ctx.rotate(rotation * Math.PI / 180);
            
            // Draw gambar (Simulasi zoom/pembesaran di tengah)
            ctx.drawImage(
                tempImg,
                -imgWidth / 2, // Mulai dari tengah
                -imgHeight / 2,
                imgWidth,
                imgHeight
            );
            ...
            // Proses Transparansi (Menggunakan logika makeImageTransparent yang sudah ada di luar, 
            // namun di sini kita lakukan secara manual untuk canvas to dataURL)
            const dataUrl = canvas.toDataURL("image/png");
            
            // Catatan: Fungsi makeImageTransparent hanya menerima URL/Base64, jadi kita ulangi logika Canvas
            const transparentDataUrl = await new Promise<string>((resolve) => {
                ...
                imgFinal.onload = () => {
                    ...
                    canvasFinal.width = imgFinal.width;
                    canvasFinal.height = imgFinal.height;
                    ctxFinal.drawImage(imgFinal, 0, 0);
                    ...
                    for (let i = 0; i < data.length; i += 4) {
                        ...
                        // Hanya membuat area putih menjadi transparan, bukan area yang sangat gelap (tinta)
                ...
                imgFinal.onerror = () => resolve(dataUrl); // Fallback
            });


            // Konversi kembali Data URL menjadi File (Blob)
            const res = await fetch(transparentDataUrl);
            const blob = await res.blob();
            const croppedFile = new File([blob], originalFile.name, { type: 'image/png' });

            onCropComplete(croppedFile, transparentDataUrl);
        };
    };

    return (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-[2000]">
            <div className="bg-white p-6 rounded-lg shadow-2xl w-full max-w-xl transform transition-all">
                <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                    <Check size={24} className="text-green-600"/> Edit Tanda Tangan
                </h3>
                <p className="text-sm text-gray-600 mb-4">
                    Atur pembesaran (Zoom) dan rotasi gambar tanda tangan agar terlihat jelas.
                </p>
                
                {/* AREA CROPPING SIMULASI */}
                <div className="relative w-full h-64 bg-gray-200 flex items-center justify-center rounded-lg overflow-hidden border-4 border-dashed border-gray-400">
                    <div className="w-full h-full relative flex items-center justify-center">
                        <img 
                            src={imageUrl} 
                            alt="Tanda Tangan Untuk Crop" 
                            style={{ 
                ...
                <div className="mt-4 p-3 border rounded-lg bg-gray-50">
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        />
                        <span className="text-sm font-semibold w-8 text-right">x{scale.toFixed(1)}</span>
                    </div>
                    <div className="flex items-center gap-4">
                        <label className="text-sm font-medium w-20">Rotasi:</label>
                        <input
                            type="range"
                            min="-180"
                            max="180"
                            ...
                            className="flex-1 cursor-pointer"
                        />
                        <span className="text-sm font-semibold w-8 text-right">{rotation}°</span>
                    </div>
                </div>
...
// ... (Fungsi makeImageTransparent tetap sama)
...
        "Ref. No. 123/PDAM/IX/2025",
        "Ref. No. 456/PDAM/IX/2025",
        "Ref. No. 789/PDAM/IX/2025",
    ]);
...
    const ttdFileInputRef = useRef<HTMLInputElement>(null); 
    ...
    // Menyimpan URL file dan File asli untuk proses cropping
    const [ttdImageToCrop, setTtdImageToCrop] = useState<{ url: string, file: File } | null>(null);


const fetchTtdHistory = useCallback(async (token: string, npp: string) => {
// ... (fetchTtdHistory tetap sama)
    try {
...
// ... (fetchPengajuanDetail tetap sama)
        try {
...
    // ... (Logika satker otomatis tetap sama)
useEffect(() => {
...
    // --- LOGIC TTD BARU ---
    
    // Handler untuk manual file upload (FALLBACK/UPLOAD BARU)
    const handleTtdFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (!e.target.files?.[0]) return;
        const file = e.target.files[0];
        const previewUrl = URL.createObjectURL(file);
        
        // Simpan File dan URL ke state Cropper
        setTtdImageToCrop({ url: previewUrl, file: file });
        setIsCropperModalOpen(true); // <--- BUKA MODAL CROPPING

        // Reset input value agar user bisa memilih file yang sama lagi jika perlu
        e.target.value = ''; 
    };

    // Handler setelah cropping selesai
    const handleCropComplete = (croppedFile: File, transparentDataUrl: string) => {
        setTtdPelaporFile(croppedFile);
        setTtdPelaporPreview(transparentDataUrl);
        setIsCropperModalOpen(false); // Tutup modal
        setTtdImageToCrop(null); // Bersihkan state
        setNotification({ type: 'success', message: 'Tanda tangan berhasil di-crop dan diterapkan.' });
    };
...
        if (ttdHistory.length > 0 && !ttdPelaporPreview) { // Tampilkan riwayat jika ada dan belum ada TTD terpasang
            setIsTtdHistoryModalOpen(true);
        } else {
            // Tidak ada riwayat, atau ingin ganti/ulang: langsung trigger input file manual
...
            if (ttdPelaporFile) {
                // Jika ttdPelaporFile ada, berarti itu hasil crop terbaru yang harus diupload
                formDataToSend.append('ttdPelapor', ttdPelaporFile);
            }
            else if (ttdPelaporPreview && !isEditMode) {
                // Jika ttdPelaporFile tidak ada, tapi Preview ada (dari history), 
                // kirim path Data URL (ini harus ditangani oleh API backend Anda untuk disimpan/diproses)
...
            />
            {/* INI ADALAH MODAL BARU */}
            <TtdCropperModal
                isOpen={isCropperModalOpen}
                imageUrl={ttdImageToCrop?.url || null}
                originalFile={ttdImageToCrop?.file || null}
                onCropComplete={handleCropComplete}
                onClose={() => {
                    setIsCropperModalOpen(false);
                    setTtdImageToCrop(null);
                    // PENTING: Jika dibatalkan, biarkan input file tetap kosong
                }}
            />
...
// ... (Header dan Tombol Cetak tetap sama)
...
                                        accept="image/*"
                                        onChange={handleTtdFileUpload} // <--- MODIFIKASI DI SINI
                                        className="hidden"
                                    />
                                </div>
                            )}
