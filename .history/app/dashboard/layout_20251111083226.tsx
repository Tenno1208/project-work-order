    ...
    const [userData, setUserData] = useState<{ nama: string; email: string } | null>(null);
        ...
        const [userData, setUserData] = useState<{ npp?: string; nama?: string; email?: string }>({});
            ...
            <div className="flex min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
                {/* SIDEBAR */}
                <div className={`${ collapsed ? "w-20" : "w-80" } bg-white border-r border-blue-100 transition-all
                    duration-300 flex flex-col shadow-xl relative`}>
                    {/* Header Sidebar */}
                    <div className="px-6 py-7 border-b border-blue-100/60 bg-gradient-to-r from-blue-50 to-white">
                        <div className="flex items-center gap-4">
                            <div
                                className="bg-gradient-to-br from-blue-600 to-blue-700 p-3 rounded-2xl shadow-lg flex-shrink-0 relative overflow-hidden">
                    ...
                    <nav className="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
                        ...
                        return (
                        <div key={item.href}>
                            <div className={`group flex items-center gap-4 px-5 py-4 rounded-2xl transition-all
                                duration-300 w-full relative overflow-hidden ${ active
                                ...
                                <div onClick={()=> handleNavigation(item.href)}
                                    ...
                                    >
                                    <div className={`${active ? "scale-110" : "group-hover:scale-110" }
                                ...
                                <button onClick={(e)=> {
                                    ...
                                    >
                                    <ChevronRight size={16} className={`transition-transform duration-300 ${isExpanded
                                        ? 'rotate-90' : '' } ${active ? 'text-white' : 'text-slate-500' }`} />
                                </button>
                                )}
                            </div>
                            {/* Sub Menu */}
                            ...
                            <div className="ml-8 mt-2 space-y-1">
                                ...
                                return (
                                <button key={subItem.href} onClick={()=> handleNavigation(subItem.href)}
                                    className={`group flex items-center gap-3 px-4 py-3 rounded-xl transition-all
                                    duration-300 w-full relative overflow-hidden ${
                                    subActive
                                    ? "bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-md
                                    shadow-blue-500/20 scale-105"
                                    : "hover:bg-blue-50 text-slate-600 hover:text-blue-600"
                                    }`}
                                    >
                                    <div className={`${subActive ? "scale-110" : "group-hover:scale-110" }
                                        transition-transform duration-300`}>
                ...
                <main className="flex-1 overflow-auto">
                    {/* Header Section */}
                    <div
                        className="bg-gradient-to-r from-blue-600 via-blue-700 to-blue-800 shadow-lg relative overflow-hidden">
                        ...
                        <div className="relative z-10 px-8 py-6">
                            <div className="flex items-center justify-between gap-6">
                                ...
                                <div className="flex items-center gap-4 flex-1">
                                    <div
                                        className="bg-white/20 backdrop-blur-sm p-4 rounded-2xl shadow-xl border border-white/30">
                                        {currentPage.icon &&
                                    ...
                                    <div>
                                        ...
                                        <p className="text-blue-100 text-sm font-medium flex items-center gap-2">
                                            ...
                                            {currentPage.description || (currentPage.parent ?
                                ...
                                {/* Date Display */}
                                <div
                                    className="bg-white/20 backdrop-blur-sm px-4 py-3 rounded-xl border border-white/30 shadow-lg">
                                ...
                                {/* Notification Bell */}
                                <button
                                    className="bg-white/20 backdrop-blur-sm p-3 rounded-xl hover:bg-white/30 transition-all duration-300 border border-white/30 shadow-lg relative group">
                                    <Bell className="text-white" size={20} />
                                    <span
                                        className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                                        3
                                    </span>
                                </button>
                            </div>
