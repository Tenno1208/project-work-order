...
// Mengganti History dengan Eye (icon umum untuk Data Pengajuan) atau icon lain sesuai kebutuhan
...
type ApiPengajuanItem = {
    ...
    created_at: string; 
    updated_at: string;
    // Menambahkan field ini agar sesuai dengan struktur API aslinya
    name_pelapor?: string;
    npp_pelapor?: string;
    // Jika perlu
    nama_jenis?: string; 
...
type Pengajuan = {
    ...
    uuid: string; 
    tanggal: string;
    hal: string; // Menggunakan keterangan/catatan sebagai Hal
    name_pelapor: string;
    status: string;
};
...
export default function DataPengajuanPage() {
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
            try {
                ...
                const mappedData: Pengajuan[] = activeData.map(item => ({
                    ...
                    // Menggunakan keterangan sebagai "Hal" jika ada, jika tidak, gunakan catatan
                    hal: item.keterangan || item.catatan || item.kode_barang || 'Tidak Ada Keterangan', 
                    // Menggunakan name atau npp
                    name_pelapor: item.name || item.npp || 'Anonim', 
    ...
    const handleBuatPengajuan = () => {
        ...
        if (typeof window !== 'undefined') {
            // Kita tetap menyimpan nomor surat, walau tidak ditampilkan, untuk logika di halaman 'tambah'
    ...
    const handleView = (uuid: string) => {
        router.push(`/dashboard/lampiran/view/${uuid}`); // Ubah path ke view detail
    };
    
    const handleEdit = (uuid: string) => {
        // Simpan UUID di localStorage agar halaman edit dapat mengambil data detail
    ...
    const getStatusStyle = (status: string) => {
        switch (status.toLowerCase()) {
            ...
            case "diproses":
            case "selesai": // Mengganti status 'Selesai' di riwayat sebelumnya agar konsisten dengan blue
            case "": 
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-gray-900 mb-1">
                                Data Pengajuan Perbaikan
                            </h2>
                            <p className="text-gray-700 text-sm">
                                Daftar seluruh pengajuan perbaikan teknis dari unit/satker PDAM
                            </p>
                        </div>
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full">
                        <thead className="bg-gradient-to-r from-blue-50 to-blue-100">
                            <tr>
                                <th className="px-4 py-3 text-left text-xs font-bold text-gray-900 uppercase tracking-wider w-[5%]">No</th>
                                <th className="px-4 py-3 text-left text-xs font-bold text-gray-900 uppercase tracking-wider w-[15%]">Tanggal</th>
                                <th className="px-4 py-3 text-left text-xs font-bold text-gray-900 uppercase tracking-wider w-[30%]">Hal (Keterangan)</th>
                                <th className="px-4 py-3 text-left text-xs font-bold text-gray-900 uppercase tracking-wider w-[25%]">Pelapor</th>
                                <th className="px-4 py-3 text-left text-xs font-bold text-gray-900 uppercase tracking-wider w-[15%]">Status</th>
                                <th className="px-4 py-3 text-center text-xs font-bold text-gray-900 uppercase tracking-wider w-[10%]">Aksi</th>
                            </tr>
                        </thead>
                                ...
                                filtered.map((p, i) => (
                                    <tr
                                        ...
                                        className={`hover:bg-blue-50/50 transition-colors duration-200 ${
                                            i % 2 === 0 ? "bg-white" : "bg-blue-25" // Warna selang-seling
                                        }`}
                                    >
                                        ...
                                        <td className="px-4 py-3 text-xs font-medium whitespace-nowrap">
                                            <div className="flex justify-center space-x-2">
                                                ...
                                                <button
                                                    ...
                                                    title="Hapus Pengajuan (menggunakan UUID)"
                                                ...
                                                {/* Tombol Download (Ditambahkan untuk konsistensi, walau fungsinya belum diimplementasi di sini) */}
                                                {/* <button
                                                    onClick={() => alert('Fungsi download belum diimplementasi')}
                                                    title="Download Dokumen"
                                                    className="p-1.5 bg-green-100 hover:bg-green-200 rounded-md transition-colors duration-200 disabled:opacity-50"
                                                >
                                                    <Download className="text-green-700 hover:scale-110 transition-transform" size={14} />
                                                </button> */}
                                            </div>
                                        </td>
                                    </tr>
