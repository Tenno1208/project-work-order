...
            // Simulate page change using console log, as required in the single-file environment.
        }
    }, []);
    return { push };
};
...
// --- KONSTANTA API ---
// LIST API PATH, asumsi menggunakan jalur proxy internal Next.js
const LIST_API_PATH = "/api/pengajuan"; 
// DELETE API menggunakan ID (sesuai implementasi URL Anda)
...
    is_deleted: number; // Field ini digunakan untuk filter di fetching
    created_at: string; 
    updated_at: string;
};
...
type Pengajuan = {
    id: number;
    uuid: string; // Sudah dipertahankan
    tanggal: string;
    hal: string;
    pelapor: string;
    status: string;
...
        
        // Menggunakan toLocaleDateString lebih aman dan tetap menghasilkan format DD/MM/YYYY
        // yang kemudian diganti dengan strip (-)
...
    const [itemToDelete, setItemToDelete] = useState<{id: number | null, hal: string | null}>({id: null, hal: null});
...
            // Catatan: Jika API list juga diblokir oleh localtunnel (walau menggunakan proxy path), 
            // header ini juga perlu ditambahkan di sini.
        };
...
                // Filter data yang tidak dihapus secara logis (is_deleted !== 1)
...
                    setLoading(false);
                    // Menambahkan data error agar pengguna tahu ada masalah
                    setPengajuans([
                        { id: 0, uuid: 'error', tanggal: "00-00-0000", hal: "GAGAL MEMUAT DATA ASLI (Jaringan/Server)", pelapor: "System Error", status: "Error" },
                    ]);
                }
            }
        }
...
    useEffect(() => {
        fetchData();
    }, [showToast]); // Tambahkan showToast sebagai depedensi jika Anda ingin me-refresh setelah aksi sukses/gagal
...
    // Menggunakan UUID untuk navigasi halaman View
    const handleView = (uuid: string) => {
        router.push(`/dashboard/lampiran/${uuid}`); // Navigasi ke halaman detail
    };
    
...
    const handleDeleteClick = (id: number, hal: string) => {
        setItemToDelete({ id, hal });
...
            setIsDeleting(false);
            setItemToDelete({id: null, hal: null});
            return;
        }

        const headers: HeadersInit = {
            'Authorization': `Bearer ${token}`,
            // FIX PENTING: Tambahkan header ini untuk bypass halaman pengamanan localtunnel
            'bypass-tunnel-reminder': 'true' 
        };

        const deleteUrl = `${DELETE_API_BASE}/${idToDelete}`;
...
                // API delete seringkali mengembalikan 200/204, kita cek statusnya
                if (!response.ok) {
                    // Coba parse body untuk pesan error yang lebih detail
                    let errorBody: any = await response.text(); 
                    try {
...
                    }
                    
                    // Status 401 dan 511 (Network Auth Required) sama-sama indikasi masalah otorisasi/jaringan
                    // Sekarang kita tahu 401 mungkin dari localtunnel, tapi kita tangkap sebagai error
                    if (response.status === 401 || response.status === 511) {
                        throw new Error(`Otorisasi Gagal: Status ${response.status}. ${typeof errorBody === 'object' ? errorBody.message : 'Token tidak valid, localtunnel memblokir, atau jaringan bermasalah.'}`);
                    }
                    throw new Error(`Gagal menghapus data. Kode Status: ${response.status}. Pesan: ${typeof errorBody === 'object' ? errorBody.message : response.statusText}`);
                }
                
                // Jika berhasil, perbarui tampilan secara lokal
                setPengajuans(prev => prev.filter(p => p.id !== idToDelete));
                console.log(`[SUCCESS]: Data pengajuan ID ${idToDelete} berhasil dihapus dari API.`);
...
                if (i === MAX_RETRIES - 1 || error.message.includes("Otorisasi Gagal") || error.message.includes("Status 511")) {
                    const errorMsg = `Gagal total menghapus data ID ${idToDelete}: ${error.message}`;
                    setAuthError(errorMsg);
                    // NOTIFIKASI GAGAL BARU
                    showToast(`Gagal menghapus: ${error.message.substring(0, 100)}...`, 'error');
                    setIsDeleting(false);
                    setItemToDelete({id: null, hal: null});
                    return; 
                } else {
...
                        <p>Apakah Anda yakin ingin menghapus data pengajuan dengan ID **{id}** ({hal})?</p>
                        <p className="text-sm mt-2 text-gray-500">Tindakan ini tidak dapat dibatalkan dan akan mengirim permintaan DELETE ke API eksternal.</p>
                    </div>
...
            <DeleteConfirmationModal />
            <Toast /> {/* Tambahkan komponen Toast di sini */}
...
            {/* Tambahkan peringatan otorisasi/aksi */}
            {authError && (
...
                                            {/* Action: DELETE (Hapus Data) - Menggunakan ID untuk API Delete Anda */}
                                            <button
                                                onClick={() => handleDeleteClick(p.id, p.hal)}
                                                title="Hapus Data (menggunakan ID)"
                                                disabled={isDeleting}
