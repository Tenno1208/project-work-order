...
export default function SPKPage() {
  ...
  const [jenisPekerjaan, setJenisPekerjaan] = useState<
    { id: number; nama_pekerjaan: string }[]
  >([]);
...
useEffect(() => {
  const fetchJenisPekerjaan = async () => {
    try {
      ...
      console.log("✅ Data jenis pekerjaan:", json);

      // Ambil langsung array dari json.data
      if (json && Array.isArray(json.data)) {
        setJenisPekerjaan(json.data);
      } else {
        console.warn("⚠️ Format tidak sesuai:", json);
      }
    } catch (err) {
                  ...
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className="border border-gray-500 rounded-md bg-white text-sm py-1 px-2 w-full focus:ring-2 focus:ring-blue-300 text-black"
                      >
                ...
                >
                  ...
                  <div className="flex mt-3 items-center">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="border border-gray-400 rounded-md bg-white outline-none px-3 py-1 text-sm w-[180px] focus:ring-2 focus:ring-blue-300 text-black"
                      />
                      <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 w-4 h-4 pointer-events-none" />
                    </div>
                  </div>

                  {/* ✅ Dropdown Jenis Pekerjaan dari API */}
                  <div className="flex mt-4 items-center">
                    ...
                    <select
                      ...
                      className="border border-gray-400 rounded-md bg-white outline-none px-2 py-1 text-sm flex-1 focus:ring focus:ring-blue-300 text-black"
                    >
                      <option value="">-- Pilih Jenis Pekerjaan --</option>
                      {jenisPekerjaan.length === 0 ? (
                        <option disabled>Memuat data...</option>
                      ) : (
                        jenisPekerjaan.map((jp) => (
                          <option key={jp.id} value={jp.nama_pekerjaan}>
                            {jp.nama_pekerjaan}
                          </option>
                        ))
                      )}
