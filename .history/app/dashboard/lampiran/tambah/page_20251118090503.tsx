...
import { Droplet, Printer, Upload, X, Check, AlertTriangle, Loader2, PlusCircle } from "lucide-react"; 
...
export default function LampiranPengajuanPage() {
    ...
    const fetchPengajuanDetail = useCallback(async (token: string, uuid: string, halOptions: HalOption[], satkers: SatkerDef[]) => {
        ...
        try {
            ...
            // Set nomor surat khusus untuk mode edit (optional, tapi dihilangkan dari tampilan)
            // localStorage.setItem(`nomor_surat_${uuid}`, `SPK-EDIT-${data.id}`);

            // Handle file yang sudah ada
if (data.file_paths) {
    try {
        // 1. Sanitasi string dari garis miring yang berlebihan
        // Mengganti semua \ (escaped atau tidak) dan // menjadi / tunggal
        let safePathString = data.file_paths
            .replace(/\\/g, '/') // Ganti semua backslash dengan forward slash
            .replace(/\/\//g, '/'); // Ganti double slash dengan single slash

        // 2. Coba parse sebagai JSON (Jika backend kirim array JSON stringified)
        let existingFiles: string[];
        try {
            existingFiles = JSON.parse(safePathString);
            if (!Array.isArray(existingFiles)) {
                throw new Error("Not an array");
            }
        } catch {
            // 3. Jika gagal parse, asumsikan string dipisahkan koma dan pecah
            existingFiles = safePathString.split(',').map(s => s.trim()).filter(s => s.length > 0);
        }

        // 4. Tambahkan base URL/path ke file path yang relatif (Jika diperlukan)
        const baseUrl = EXTERNAL_EDIT_BASE_URL.endsWith('/') ? EXTERNAL_EDIT_BASE_URL.slice(0, -1) : EXTERNAL_EDIT_BASE_URL;
        
        const finalFilePaths = existingFiles.map(path => {
            // Jika path belum memiliki protokol (http/https), tambahkan base URL.
            // Gunakan path.replace(/^\//, '') untuk menghapus garis miring di awal path jika ada.
                ...
                <div id="print-area" className="p-6">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="text-right text-sm">
                            {/* NOMOR SURAT DIHILANGKAN DARI TAMPILAN */}
