...
function generateDynamicPath(type: 'ttd' | 'pengajuans'): string {
    ...
    // PATH HARUS DIAKHIRI DENGAN GARIS MIRING /
...
async function uploadFile(file: File, token: string, customName: string, type: 'ttd' | 'pengajuans'): Promise<string> {
    ...
    // Pastikan customName TIDAK mengandung path
    const fileNameOnly = customName.split('/').pop() || customName; 
    
    formData.append('photo', file, fileNameOnly); 
    formData.append('path', uploadPath); 
    formData.append('filename', fileNameOnly); // Kirim hanya nama file
...
// -------------------------------------------------------------------------
// FUNGSI 2: uploadMultipleFiles (Untuk Lampiran > 1) - PERBAIKAN UTAMA
// -------------------------------------------------------------------------
async function uploadMultipleFiles(files: File[], token: string, customNamePrefix: string): Promise<string[]> {
    ...
    formData.append('path', uploadPath); // PATH yang akan digunakan untuk membuat folder
    formData.append('photo_count', files.length.toString());
    
    for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const fileExt = file.name.split('.').pop() || 'jpg';
        // HANYA NAMA FILE TANPA PATH
        const fileNameOnly = `${customNamePrefix}-${i}.${fileExt}`; 
        
        // photo_N: upload file
        formData.append(`photo_${i + 1}`, file, fileNameOnly); 
        
        // filename_N: Sesuai screenshot Postman, ini mungkin HANYA NAMA FILE unik
        formData.append(`filename_${i + 1}`, fileNameOnly); // PERBAIKAN INTI
    }
...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Harap ganti ini dengan UUID dan NPP yang sebenarnya
        ...
        if (ttdUrl) {
            ...
            // Ini akan mengirim nilai ttdPelaporPath jika ada (digunakan di mode edit/update)
