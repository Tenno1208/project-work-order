...
// Endpoint eksternal untuk mengambil daftar SPK.
const EXTERNAL_SPK_LIST_URL = `${API_BASE_URL}/api/spk/lihat/data`;

/**
 * Handle GET request untuk mengambil daftar SPK dari API eksternal.
 * Rute lokal: /api/spk/list
 */
export async function GET(req: Request) {
    // 1. Ambil Token Otorisasi dari header request klien
    ...
    try {
        ...
        // 2. Lakukan fetch ke API eksternal
        const res = await fetch(EXTERNAL_SPK_LIST_URL, {
            headers: {
                Accept: "application/json",
                Authorization: `Bearer ${token}`,
                // Bypass header untuk ngrok atau tool lokal lainnya
        ...
        // 3. Mengembalikan data (diasumsikan data SPK berada di result.data)
        return NextResponse.json({
            success: true,
