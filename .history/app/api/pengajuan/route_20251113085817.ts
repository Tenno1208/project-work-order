...
async function uploadFile(file: File, token: string, customName: string): Promise<string> {
    ...
    // Karena file handler Anda memerlukan path dan filename, 
    // Kita asumsikan itu juga perlu dikirimkan, mirip Postman Anda.
    // Jika tidak tahu path apa yang harus dikirim, ini mungkin bisa dihilangkan, 
    // atau gunakan path default, misalnya 'work-order/default/'
    formData.append('path', 'work-order/2025/11/'); // Contoh Path
