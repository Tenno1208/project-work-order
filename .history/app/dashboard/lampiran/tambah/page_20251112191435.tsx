...
export default function LampiranPengajuanPage() {
  ...
  const [form, setForm] = useState({
    ...
    pelapor: "",
    nppPelapor: "",
    mengetahui: "", // Field ini akan diisi dari API lain nanti
    nppMengetahui: "", // Field ini akan diisi dari API lain nanti
  });
  ...
  const handleAjukan = async () => {
    // Validasi field wajib (mengetahui dan nppMengetahui DIHAPUS dari validasi wajib)
    const wajibDiisi = [
      { field: "hal", label: "Hal" },
    ...
    try {
      // Pastikan token diambil dari localStorage dan disertakan di header
      const token = localStorage.getItem("token");
      if (!token) {
      ...
      const res = await fetch("/api/pengajuan", {
        method: "POST",
        headers: {
            "Authorization": `Bearer ${token}`, // Mengirim token melalui header
        },
