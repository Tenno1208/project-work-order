...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
     ...
     */
    // Di dalam DashboardLayout.js:
const handleLogout = async () => {
    setLoggingOut(true);
    const token = localStorage.getItem("token");

    // Hapus token lokal SEBELUM redirect, tapi setelah request dikirim.
    localStorage.removeItem("token"); 
    ...
    try {
        ...
        if (res.ok || res.status === 401) {
             // Status 401 (Unauthorized) dari API eksternal juga dianggap success logout sesi, 
             // karena tujuannya sudah tercapai: client dan server tidak terhubung.
        ...
        // Selalu redirect ke halaman login setelah token lokal dihapus
        router.push("/login");
        setLoggingOut(false);
    }
};
