...
async function uploadFile(file: File, token: string, customName: string, type: 'ttd' | 'pengajuans'): Promise<string> {
    ...
    // --- PERUBAHAN: Buat path lengkap di sini ---
    const now = new Date();
    const year = now.getFullYear();
    const month = (now.getMonth() + 1).toString().padStart(2, '0');
    const fullPath = `work-order/${year}/${month}/${customName}`;
    
    formData.append('photo', file);
    // --- PERUBAHAN: Kirim fullPath di filename ---
    ...
    if (res.ok && result.data?.filepath) { 
        ...
        // --- PERUBAHAN: Kembalikan path lengkap ---
...
async function uploadMultipleFiles(files: File[], token: string, customNamePrefix: string): Promise<string[]> {
    ...
    // --- PERUBAHAN: Buat path dasar di sini ---
    const now = new Date();
    const year = now.getFullYear();
    const month = (now.getMonth() + 1).toString().padStart(2, '0');
    const basePath = `work-order/${year}/${month}`;
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        // --- PERUBAHAN: Buat path lengkap di filename ---
        const fullPath = `${basePath}/${fileName}`;
        
        formData.append(`photo_${i + 1}`, file);
        formData.append(`filename_${i + 1}`, fullPath);
    }
    ...
    if (res.ok && result.data && Array.isArray(result.data)) { 
        ...
        // --- PERUBAHAN: Kembalikan array filepath ---
...
export async function POST(req: Request) {
    ...
    try {
        ...
        // 1. AKTIFIKAN UPLOAD TTD PELAPOR (Satu File API)
