...
useEffect(() => {
    const satkerId = form.satker; // ID Satker (string)
    const selectedSatker = satkers.find(s => s.id === satkerId);
    const token = localStorage.getItem("token") || ''; // Ambil token untuk potential fallback
    
    // Pastikan Satker & Pegawai dimuat
    if (!satkerId || !selectedSatker || allPegawai.length === 0) {
        setForm(prev => ({ ...prev, mengetahui: "", nppMengetahui: "" }));
        return;
    }

    const targetJabatanSatker = selectedSatker.jabatan.toLowerCase(); 
    
    // --- STRATEGI PENCARIAN BARU (Membalik Logika Pencarian) ---
    
    const penanggungJawab = allPegawai.find(peg => {
        const pegSatkerId = String(peg.satker_id);
        const pegJabatan = peg.jabatan.toLowerCase();

        // 1. Harus berada di Satker yang sama
        if (pegSatkerId !== satkerId) {
            return false;
        }

        // 2. Kriteria Jabatan:
        // Cek jika Jabatan Pegawai (yang lebih umum: "kepala sub bagian") 
        // terkandung di dalam Jabatan Satker (yang lebih spesifik: "kepala sub bagian humas dan protokol").
        // ATAU, jika Jabatan Pegawai adalah Kepala Unit, Kepala Bagian, dll.

        // OPSI A (Pencocokan Substring): Apakah Jabatan Pegawai (pegJabatan) adalah awal dari Jabatan Satker (targetJabatanSatker)?
        // Contoh: "kepala sub bagian humas dan protokol".startsWith("kepala sub bagian") -> TRUE
        if (targetJabatanSatker.startsWith(pegJabatan)) {
            return true;
        }

        // OPSI B (Fallback untuk kepala/pimpinan): Jika jabatan di pegawai mengandung "kepala" atau "kabag"
        if (pegJabatan.includes('kepala') || pegJabatan.includes('kabag') || pegJabatan.includes('kasi')) {
             // Ini adalah filter yang longgar, hanya jika OPSI A gagal
             return true; 
        }
    ...
    if (penanggungJawab) {
        ...
        // Fallback: Kosongkan dan beri notifikasi jika tidak ketemu
        setForm(prev => ({ ...prev, mengetahui: "", nppMengetahui: "" }));
        setNotification({ type: 'warning', message: `Tidak ditemukan Penanggung Jawab (${selectedSatker.jabatan}) di Satker ini. Harap isi manual.` });
    }
}, [form.satker, satkers, allPegawai, setNotification]);


// ... (sisa code di LampiranPengajuanPage function)
