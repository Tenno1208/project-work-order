...
const LOGOUT_API_URL = "https://gateway.pdamkotasmg.co.id/api-gw-balanced/api/auth/logout";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const [loggingOut, setLoggingOut] = useState(false); // State baru untuk loading logout
    ...
    const handleLogout = async () => {
        ...
        // Hapus token lokal segera setelah permintaan dikirim, terlepas dari hasil API.
        // Ini adalah praktik umum untuk mencegah sesi yang rusak.
        localStorage.removeItem("token");

        if (!token) {
            console.log("Token tidak ditemukan, langsung redirect.");
            router.push("/login");
            setLoggingOut(false);
            return;
        }
        ...
        try {
            const res = await fetch(LOGOUT_API_URL, {
                method: "POST", // Umumnya logout adalah POST
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`,
                },
                // Pastikan tidak ada body jika tidak diperlukan
                // body: JSON.stringify({}) 
            });

            if (res.ok) {
                console.log("Logout API success. Redirecting...");
            } else {
                // Walaupun API gagal, token lokal sudah dihapus, jadi kita tetap log out di sisi client.
                console.error(`Logout API failed (Status: ${res.status}). Continuing local logout.`);
            }

        } catch (error) {
            console.error("Error calling logout API:", error);
            // Error jaringan/server, tetap lanjutkan logout lokal.
        } finally {
            // Selalu redirect ke halaman login setelah mencoba API, karena token sudah dihapus.
            router.push("/login");
            setLoggingOut(false);
        }
    };
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] backdrop-blur-sm transition-opacity">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 w-[90%] max-w-xs text-center relative overflow-hidden transform transition-all scale-100"> 
                        ...
                        <p className="text-gray-500 text-sm mb-6">
                            Anda yakin ingin keluar dari sistem? Ini akan memanggil API logout.
                        </p>
