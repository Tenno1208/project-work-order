...
// --- TYPES SESUAI API (Lokasi dihapus) ---
type SPKItem = {
    id: number; // ID internal database
    nomor: string; // Nomor SPK/UUID (Mengambil dari no_surat)
    pekerjaan: string; // Jenis/Uraian Pekerjaan
    tanggal: string; // Tanggal SPK
    status: "Selesai" | "Proses" | "Menunggu" | string; // Status SPK
    uuid: string; // UUID untuk aksi edit/delete/view (Mengambil dari uuid_pengajuan)
    namaPetugas?: string; // Penanggung Jawab Name
    // lokasi?: string; // Dihapus
};
...
export default function RiwayatSPKPage() {
    ...
    // ---- FETCH DATA LIST SPK (Mapping disesuaikan) ----
    const fetchData = async () => {
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
            try {
                ...
                // Mapping data dari API ke format SPKItem (disesuaikan dengan JSON baru)
                const mapped: SPKItem[] = result.data.map((item: any) => ({
                    id: item.id,
                    // Mengambil no_surat. Jika kosong, gunakan uuid_pengajuan
                    nomor: (item.no_surat || item.uuid_pengajuan)?.toString() || "N/A",
                    // Mengambil dari uraian_pekerjaan atau jenis_pekerjaan
                    ...
                    // Menggunakan uuid_pengajuan untuk identifikasi unik aksi
                    uuid: item.uuid_pengajuan.toString() || item.id.toString(),
                    
                    // Mengambil nama Penanggung Jawab
                    namaPetugas: item.penanggung_jawab_name || "-", 
                    
                    // lokasi: item.lokasi_pekerjaan || item.lokasi || "-", // Dihapus
                }));
                                    ...
                                    {filteredData.length === 0 ? (
                                        ...
                                        filteredData.map((item, index) => (
                                            <tr key={item.uuid} className="hover:bg-blue-50 transition-colors">
                                                ...
                                                {/* <td className="px-4 py-3 text-xs text-black whitespace-nowrap">{item.lokasi}</td> // Dihapus */}
