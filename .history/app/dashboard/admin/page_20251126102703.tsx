...
const ALL_PEGAWAI_API_PATH = '/api/all-pegawai'; // List semua pegawai
const USER_PERMS_API_PATH = '/api/admin/user-permissions'; // API untuk GET/SET permission user lain (ASUMSI BARU)
...
export default function AdminDashboard() {
    ...
    const loadSystemPermissions = useCallback(async (token: string) => {
        try {
            ...
            if (!rawPermsJson || rawPermsJson.trim().length === 0) return;
            
            const rawPermissions = JSON.parse(rawPermsJson) as string[];
            
            // Cek Izin Admin (misal: hanya yang punya izin 'master-users' boleh akses)
            const requiredAdminPermission = 'portal-pegawai.settings.admin.master-users';
            if (rawPermissions.includes(requiredAdminPermission)) {
                setIsUserAdmin(true);
            } else {
                // Jika bukan admin, kita tidak perlu melanjutkan proses load permission
                return;
            }
            ...
            console.error("Error loading system permissions:", e);
        }
    }, []);
    ...
    const fetchTargetUserPermissions = useCallback(async (token: string, npp: string) => {
        ...
        try {
            ...
            if (json.success && Array.isArray(json.data.permissions)) {
                ...
                // Jika API gagal atau user tidak punya permission sama sekali
                setUserToEditPermissions([]);
    ...
    const savePermissions = async () => {
        ...
        try {
            ...
            const res = await fetch(USER_PERMS_API_PATH, {
                ...
                body: JSON.stringify({
                    npp: selectedUserNPP,
                    permissions: userToEditPermissions,
                }),
            });
    ...
    useEffect(() => {
        ...
        // 2. Muat daftar user (jika user adalah admin)
        if (isUserAdmin) {
            fetchAllUsers(token);
        }

        setIsLoading(false);
    }, [router, isUserAdmin, loadSystemPermissions, fetchAllUsers]);
    
    // --- USE EFFECT UNTUK USER YANG DIPILIH ---
    useEffect(() => {
        const token = localStorage.getItem('token');
        if (selectedUserNPP && token) {
            fetchTargetUserPermissions(token, selectedUserNPP);
        }
    }, [selectedUserNPP, fetchTargetUserPermissions]);
                                        ...
                                        <div className="p-4 bg-white">
                                            ...
                                            {isLoading ? (
                                                <div className="py-4 text-center text-gray-500">
                                                    <Loader2 className="animate-spin inline-block mr-2" size={20}/> Memuat izin pengguna target...
                                                </div>
                                            ) : (
