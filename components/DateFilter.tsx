...
const DateFilter: React.FC<DateFilterProps> = ({ 
    ...
    const presetOptions = [
        { value: 'today', label: 'Hari ini', icon: <Clock size={14} className="text-black" />, desc: 'Data hari ini' },
        { value: 'this_week', label: 'Minggu ini', icon: <Calendar size={14} className="text-black" />, desc: 'Senin - Minggu' },
        { value: 'last_week', label: 'Minggu lalu', icon: <Calendar size={14} className="text-black" />, desc: '7 hari terakhir' },
        { value: 'this_month', label: 'Bulan ini', icon: <Calendar size={14} className="text-black" />, desc: 'Bulan berjalan' },
        { value: 'last_month', label: 'Bulan lalu', icon: <Calendar size={14} className="text-black" />, desc: 'Bulan sebelumnya' },
        { value: 'this_quarter', label: 'Kuartal ini', icon: <TrendingUp size={14} className="text-black" />, desc: 'Q1/Q2/Q3/Q4' },
        { value: 'last_quarter', label: 'Kuartal lalu', icon: <TrendingUp size={14} className="text-black" />, desc: 'Kuartal sebelumnya' },
        { value: 'this_year', label: 'Tahun ini', icon: <Calendar size={14} className="text-black" />, desc: 'Tahun berjalan' },
        { value: 'last_year', label: 'Tahun lalu', icon: <Calendar size={14} className="text-black" />, desc: 'Tahun sebelumnya' },
    ];
    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-3 mb-4 border border-black"> {/* Changed border-gray-200 to border-black */}
            {/* Filter Controls */}
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold text-black">Filter Periode</h3>
                </div>
                {isFilterActive() && (
            ...
            <div className="relative">
                <button
                    ...
                    className="w-full px-3 py-2 border border-black rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-black flex items-center justify-between hover:border-black transition-all text-sm"
                >
                    <span className="flex items-center gap-2">
                        {filter.preset && presetOptions.find(o => o.value === filter.preset)?.icon}
                        <span>{getPresetLabel()}</span>
                    </span>
                    <ChevronDown size={16} className={`transition-transform text-black ${isDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                
                {isDropdownOpen && (
                    // Changed border-gray-200 to border-black
                    <div className="absolute z-20 w-full mt-1 bg-white border border-black rounded-md shadow-lg overflow-hidden">
                        {/* Changed bg-gray-50 to bg-white and border-gray-200 to border-black */}
                        ...
                        {activeTab === 'preset' ? (
                            <div className="py-1 max-h-64 overflow-y-auto">
                                {presetOptions.map((option) => (
                                    ...
                                    >
                                        {/* Changed bg-gray-100 to bg-white for contrast against gray text context if any, but using blue/white logic mostly */}
                                        <div className={`p-1 rounded ${filter.preset === option.value ? 'bg-blue-200' : 'bg-gray-100'}`}>
                                            {option.icon}
                                        </div>
                                        <div className="flex-1">
                                            <div className={`text-sm font-medium ${filter.preset === option.value ? 'text-blue-700' : 'text-black'}`}>
                                                {option.label}
                                            </div>
                            ...
                            // Changed bg-gray-50 to bg-white to remove gray background
                                ...
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            // Fixed: border-black-300 (invalid) -> border-black. Added placeholder:text-black.
                                            className="w-full px-3 py-2 border border-black rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm text-black placeholder:text-black"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-medium text-black mb-1">
                                        ...
                                        <input
                                            ...
                                            // Fixed: border-gray-300 -> border-black. Added placeholder:text-black.
                                            className="w-full px-3 py-2 border border-black rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm text-black placeholder:text-black"
                ...
                {showActivePeriod && !isDropdownOpen && (
                    // Changed border-blue-200 to border-black for consistency with "no gray" policy, though blue is usually fine. I'll keep blue-200 as it's colored, not gray.
                    <div className="mt-2 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-2 border border-blue-200">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <div className="text-xs text-black font-medium">Periode Aktif</div>
