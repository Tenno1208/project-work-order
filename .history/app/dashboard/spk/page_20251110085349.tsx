...
export default function SPKPage() {
  ...
  const [data, setData] = useState({ nama: "", menugaskan2: "", menugaskan3: "", tanggalSelesai: "", idBarang: "", uraianPekerjaan: "", status: "" });
  ...
  const EditableLine = ({ value, placeholder, onChange }: any) => (
    <div contentEditable suppressContentEditableWarning onInput={(e: any) => onChange(e.currentTarget.textContent || "")} className="inline-block border-b border-black pb-0.5 min-w-[120px] text-black bg-white" style={{ outline: "none", minHeight: 18 }}>
      {value || placeholder || " "}
    </div>
  );

  const EditableBox = ({ value, onChange }: any) => (
    <div contentEditable suppressContentEditableWarning onInput={(e: any) => onChange(e.currentTarget.innerText || "")} className="min-h-[140px] p-2 text-black bg-white" style={{ outline: "none", whiteSpace: "pre-wrap" }}>
      {value || " "}
    </div>
  );
  ...
  return (
    <div className="p-6 min-h-screen bg-gray-100">
      <div className="max-w-4xl mx-auto">
        ...
        <div ref={docRef} className="bg-white border border-gray-300 p-8 text-[13px] leading-relaxed font-serif shadow-md rounded-xl" style={{ width: "210mm", minHeight: "297mm" }}>
          <h2 className="text-center font-bold underline mb-4 text-black" style={{ fontSize: 16 }}>SURAT PERINTAH KERJA</h2>

          {step === 1 && (
            <div className="mt-2 text-black space-y-4">
              <div className="flex items-center">
                <div className="w-[140px]">Menugaskan Sdr</div>
                <select value={data.nama} onChange={(e) => updateField("nama", e.target.value)} className="border border-gray-400 rounded px-2 py-1">
              ...
              <Button onClick={() => setStep(2)} className="mt-4">Selesai</Button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="mt-2 text-black">
              <div className="flex mt-3">
                ...
                <EditableLine value={data.tanggalSelesai} placeholder="dd-mm-yyyy" onChange={(v) => updateField("tanggalSelesai", v)} />
              </div>
              ...
              <div className="flex mt-3">
                ...
                <EditableLine value={data.idBarang} placeholder="(ketik ID...)" onChange={(v) => updateField("idBarang", v)} />
              </div>
                ...
                <div className="flex items-center gap-8">
                  {['Selesai', 'Belum Selesai'].map((s) => (
                    <div key={s} className="flex items-center gap-2 cursor-pointer" onClick={() => toggleStatus(s)}>
                      <div style={{ width: 18, height: 18, border: '1px solid #000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{data.status === s ? '✓' : ''}</div>
                      <div>{s}</div>
                    </div>
                  ))}
                </div>
