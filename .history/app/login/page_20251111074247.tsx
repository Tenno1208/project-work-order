...
// MEMPERBAIKI ERROR: Mengganti "next/navigation" dengan "next/router"
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mencoba membaca body respons, walaupun statusnya error
      const text = await response.text();
      // Pastikan text respons bukan string kosong sebelum mencoba parse JSON
      const data = text ? JSON.parse(text) : {}; 

      console.log("Response dari /api/login:", data);

      if (!response.ok) {
        // --- LOGIKA VALIDASI UNTUK KREDENSIAL SALAH / USER TIDAK DITEMUKAN ---
        let errorMessage = "Terjadi kesalahan saat login.";

        // Jika status adalah 401 (Unauthorized) atau 400 (Bad Request),
        // gunakan pesan error yang spesifik dan aman (NPP/Password salah).
        if (response.status === 401 || response.status === 400) {
          errorMessage = "NPP atau Password salah. Silakan coba lagi.";
        } else if (data.message) {
          // Gunakan pesan error dari backend jika tersedia
          errorMessage = data.message;
        } else {
          // Pesan error umum untuk status lain
      ...
      // ✅ Ambil token dari hasil backend Next.js
