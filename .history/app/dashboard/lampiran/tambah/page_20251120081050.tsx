...
<div className="mt-4 flex gap-20">
    <div className="w-1/2 text-sm font-semibold">
        ...
        <div className="mt-2 text-sm font-semibold">
            Ref. Surat :
            {isPrintMode ? (
                <span className="ml-2 font-normal">{form.referensiSurat || "-"}</span>
            ) : (
                <Select
                    name="referensiSurat"
                    ...
                    className="ml-2 w-3/4 text-sm select-input-only"
                    styles={{
                        menu: (base) => ({ ...base, zIndex: 50, position: "absolute" }),
                        control: (base) => ({ ...base, border: '1px solid #d1d5db', borderRadius: '0.25rem', background: 'white' }),
                    }}
                    menuPlacement="auto"
                    isClearable
                />
            )}
            <div className="text-xs text-gray-500 italic mt-1 ml-2">
...
// Hapus grid "Ref. Surat :" yang lama (karena sudah dipindah ke bawah Hal)

// ... (kode setelahnya tetap sama, termasuk grid untuk Satker, Kode Barang, dll.)
