              ...
              <div key={item.href}>
                ...
                >
                  ...
                  {item.subItems && !collapsed && (
                    <button
                      onClick={(e) => { e.stopPropagation(); toggleExpand(item.href); }}
        ...
        <div className="bg-white/20 backdrop-blur-sm rounded-xl border border-white/30 shadow-lg p-3 flex items-center gap-3">
          ...
          <div className="pr-3 border-r border-white/30">
            {loading ? (
              ...
              <>
                <p className="text-white text-sm font-semibold">{userData?.nama || "Admin User"}</p>
                <p className="text-blue-100 text-xs">{userData?.npp || "-"}</p>
              </>
            )}
          </div>
          <button className="p-2 hover:bg-white/20 rounded-lg transition-colors flex-shrink-0" title="Settings">
