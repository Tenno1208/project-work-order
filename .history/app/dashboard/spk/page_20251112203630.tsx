  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 p-4"> {/* Reduced padding: p-6 -> p-4 */}
      <div className="max-w-7xl mx-auto space-y-4"> {/* Reduced spacing: space-y-6 -> space-y-4 */}
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 border border-blue-100"> {/* Reduced padding: p-6 -> p-4, rounded-3xl -> rounded-2xl */}
          <div className="flex flex-col md:flex-row gap-3"> {/* Reduced gap: gap-4 -> gap-3 */}
            ...
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-black/60" size={16} /> {/* Reduced icon size: size={20} -> size={16} */}
              <input
                type="text"
                ...
                className="w-full bg-gray-50 border border-gray-300 rounded-xl py-2 pl-10 pr-3 focus:border-cyan-500 focus:bg-white outline-none text-sm text-black transition-all" // Reduced border width, padding: py-3.5 -> py-2, pl-12 -> pl-10, rounded-2xl -> rounded-xl
              />
            </div>

            {/* Filter */}
            <div className="relative">
              <Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-black/60" size={16} /> {/* Reduced icon size */}
              <select
                value={filterStatus}
                onChange={(e) => setFilterStatus(e.target.value)}
                // Reduced padding/font size
                className="bg-gray-50 border border-gray-300 rounded-xl py-2 pl-10 pr-7 focus:border-cyan-500 focus:bg-white outline-none transition-all appearance-none cursor-pointer min-w-[160px] text-sm text-black" // Reduced border width, padding: py-3.5 -> py-2, pl-12 -> pl-10, rounded-2xl -> rounded-xl, min-w-[180px] -> min-w-[160px]
              >
        ...
        {/* TABLE */}
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-blue-100"> {/* rounded-3xl -> rounded-2xl */}
          {loading ? (
            <div className="py-12 text-center"> {/* Reduced padding: py-20 -> py-12 */}
              <Loader2 className="mx-auto animate-spin text-cyan-600 mb-3" size={32} /> {/* Reduced size: size={40} -> size={32}, mb-4 -> mb-3 */}
              <p className="text-black text-sm font-medium">Memuat data SPK...</p> {/* Reduced font size */}
            </div>
          ) : filteredSpk.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="min-w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-sm"> {/* Added text-sm */}
                    <th className="px-4 py-3 text-left font-semibold">No</th> {/* Reduced padding: px-6 py-4 -> px-4 py-3 */}
                    <th className="px-4 py-3 text-left font-semibold">Nomor SPK</th> {/* Reduced padding */}
                    <th className="px-4 py-3 text-left font-semibold">Pekerjaan</th> {/* Reduced padding */}
                    <th className="px-4 py-3 text-left font-semibold">Tanggal</th> {/* Reduced padding */}
                    <th className="px-4 py-3 text-left font-semibold">Status</th> {/* Reduced padding */}
                    <th className="px-4 py-3 text-center font-semibold">Aksi</th> {/* Reduced padding */}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {filteredSpk.map((spk, index) => (
                    <tr
                      key={spk.id}
                      className="border-b border-gray-200 hover:bg-gradient-to-r hover:from-cyan-50 hover:to-blue-50 transition-all cursor-pointer group text-sm" // Added text-sm
                      onClick={() => router.push(`/dashboard/spk/${spk.id}`)}
                    >
                      <td className="px-4 py-3 text-black">{index + 1}</td> {/* Reduced padding: px-6 py-4 -> px-4 py-3 */}
                      <td className="px-4 py-3 font-semibold text-blue-700 group-hover:text-blue-800 whitespace-nowrap">{spk.nomor}</td> {/* Reduced padding */}
                      <td className="px-4 py-3 text-black">{spk.pekerjaan}</td> {/* Reduced padding */}
                      <td className="px-4 py-3 text-black flex items-center gap-1.5 whitespace-nowrap"> {/* Reduced padding, gap-2 -> gap-1.5 */}
                        <Calendar size={14} className="text-black/60" /> {/* Reduced icon size: size={16} -> size={14} */}
                        <span className="text-xs">{spk.tanggal}</span> {/* Reduced font size: text-sm -> text-xs */}
                      </td>
                      <td className="px-4 py-3"> {/* Reduced padding */}
                        <span
                          className={`inline-flex items-center px-3 py-1 text-xs font-bold rounded-full shadow-sm ${ // Reduced padding: px-4 py-2 -> px-3 py-1, text-xs is maintained
                            spk.status === "Selesai"
                      ...
                      <td className="px-4 py-3 text-center"> {/* Reduced padding */}
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
            ...
            <div className="py-12 text-center"> {/* Reduced padding: py-20 -> py-12 */}
              <div className="bg-gray-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3"> {/* Reduced size: w-20 h-20 -> w-16 h-16, mb-4 -> mb-3 */}
                <FileText className="text-gray-400" size={32} /> {/* Reduced size: size={40} -> size={32} */}
              </div>
              <p className="text-black text-sm font-medium">Tidak ada data SPK ditemukan</p> {/* Reduced font size */}
              <p className="text-black/70 text-xs mt-1"> {/* Reduced font size: text-sm -> text-xs */}
