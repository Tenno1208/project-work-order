...
if (data.ttd_pelapor) {
    ...
    // Pastikan allPegawai sudah dimuat sebelum mencari
    if (!satkerId || !selectedSatker || allPegawai.length === 0) {
        setForm(prev => ({ ...prev, mengetahui: "", nppMengetahui: "" }));
        return;
    }

    const targetJabatanSatker = selectedSatker.jabatan.toLowerCase(); // "kepala sub bagian anggaran"
    
    // --- STRATEGI PENCARIAN BARU (Lebih Fleksibel) ---
    
    const penanggungJawab = allPegawai.find(peg => {
        const pegSatkerId = String(peg.satker_id);
        const pegJabatan = peg.jabatan.toLowerCase(); // "kepala sub bagian"
        ...
        // 2. Kriteria Jabatan:
        // Coba cocokan string pegawai dengan string satker, atau sebaliknya.
        // OPSI A: Jabatan Satker mengandung Jabatan Pegawai (Paling mungkin benar untuk data ini)
        if (targetJabatanSatker.includes(pegJabatan)) {
             return true;
        }

        // OPSI B: Jabatan Pegawai mengandung kata kunci 'kepala' dan 'sub bagian' (sebagai fallback)
        if (pegJabatan.includes('kepala') && pegJabatan.includes('sub bagian')) {
             return true;
        }
