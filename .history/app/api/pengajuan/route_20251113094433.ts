...
const EXTERNAL_API_URL = "https://zephyrean-cycloidally-triston.ngrok-free.dev/api/pengajuan";const FILE_HANDLER_URL = "https://gateway.pdamkotasmg.co.id/api-gw-balanced/file-handler/api/upload/foto";
 ...
 * 🚨 PERBAIKAN: Penanganan error yang lebih spesifik untuk kegagalan upload TTD
 */
async function uploadFile(file: File, token: string, customName: string, type: 'ttd' | 'pengajuans'): Promise<string> {
    ...
    // Tambahkan field yang dibutuhkan oleh File Handler
    ...
    const res = await fetch(FILE_HANDLER_URL, {
        method: 'POST',
        headers: {
            // HANYA Authorization yang diperlukan untuk FormData
            'Authorization': `Bearer ${token}`, 
            // Content-Type: multipart/form-data otomatis ditambahkan oleh fetch saat menggunakan FormData
        },
        body: formData,
        cache: "no-store",
    });
    ...
    if (res.ok && result.success && result.data?.fileUrl) { 
        ...
        // Tambahkan detail validasi (jika API memberikan field 'errors')
...
// --- HANDLER GET (PROXY) (Perbaikan Ngrok) ---
export async function GET(req: Request) {
    try {
        ...
        try {
            ...
            // Jika gagal parse JSON, cek apakah ini peringatan Ngrok (HTML/Teks)
             ...
             // Tangkap jika API mengembalikan HTML/format tak terduga selain Ngrok
        ...
        // Cek status HTTP dan status 'success' di body
...
export async function POST(req: Request) {
    try {
        ...
        // 🚨 DIAGNOSTIK: LOG header untuk melihat apakah token terkirim
