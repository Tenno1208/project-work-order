...
const GmailDropdown = ({ 
  ...
  const filteredItems = items.filter(item => {
    ...
    const isAlreadyAssigned = assignedPeople.some(assigned => {
      // Debug log
      console.log(`Comparing: "${assigned.name}" with "${item.name}"`);
      return assigned.name.toLowerCase() === item.name.toLowerCase();
    });
    
    const matchesInput = item.name.toLowerCase().includes(inputValue.toLowerCase());
    const shouldInclude = !isAlreadyAssigned && matchesInput;
    
    // Debug log
    console.log(`Item: ${item.name}, Already assigned: ${isAlreadyAssigned}, Matches input: ${matchesInput}, Should include: ${shouldInclude}`);
...
export default function SPKPage() {
  ...
  const getPersonDetail = (name) => {
    ...
    const found = pegawaiList.find(p => p.name.toLowerCase() === name.toLowerCase());
    console.log(`Looking for: ${name}, Found:`, found);
    return found || { npp: null, jabatan: null };
  };
  ...
  const handleAddPerson = () => {
    ...
    // Gunakan nama lengkap dari detail jika ditemukan, jika tidak gunakan input
    const fullName = detail.name || name;
    
    const newPerson = {
        name: fullName, // Gunakan nama lengkap dari API
        npp: detail.npp,
        isPic: isFirstPerson, 
    };
    ...
    console.log("Updated assigned people:", updatedPeople);
