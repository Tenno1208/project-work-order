...
export async function DELETE(
    ...
    try {
        ...
        // 5. Baca response body dengan penanganan non-JSON (untuk menghindari 'Unexpected token <')
        ...
        if (contentType && contentType.includes('application/json')) {
            try {
                responseData = await externalResponse.json();
                 ...
                 // Jika gagal parse JSON padahal Content-Type-nya JSON
            ...
            // Jika respons BUKAN JSON (misalnya HTML dari 404/500/lainnya), baca sebagai teks
            const textResponse = await externalResponse.text();
            
            // Simpan respons non-JSON untuk debugging
            responseData.message = `Respons non-JSON dari External API (Status ${externalResponse.status}): ${textResponse.substring(0, 100)}...`;
            responseData.details = textResponse; // Simpan teks lengkap untuk log
        }
