...
    }
    ...
    // =================================================================
    // >> MODIFIKASI INI: Mengambil result.data.filepath <<
    // =================================================================
...
async function uploadMultipleFiles(files: File[], token: string, customNamePrefix: string): Promise<string[]> {
    ...
    // 1. Tambahkan path dan photo_count
    formData.append('path', uploadPath); 
    formData.append('photo_count', files.length.toString());
    
    // 2. Tambahkan setiap file dengan key photo_1, photo_2, ...
    for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const fileName = `${customNamePrefix}-${i}.${file.name.split('.').pop() || 'jpg'}`;
        
        formData.append(`photo_${i + 1}`, file, fileName);
        
        // Catatan: API Multiple Foto PDAM membutuhkan filename individual
        // Tetapi karena tidak ada field filename_1, filename_2 di Postman, kita hanya mengirimkan file.
        // Jika perlu, Anda bisa menambahkan filename_1, filename_2, dst. di sini.
    }
    ...
    if (res.ok && result.data && Array.isArray(result.data)) { 
        // Asumsi: API multiple upload mengembalikan array data: [{filepath: '...'}, {filepath: '...'}, ...]
...
export async function POST(req: Request) {
    ...
    try {
        ...
        const newFiles: File[] = [];
        for (let i = 0; i < 4; i++) { // Ambil semua file baru dari 0 sampai 3
        ...
        let fileUrls: string[] = []; // Ini akan menampung filepath hasil upload
        ...
        if (newFiles.length > 0) {
            ...
            try {
                ...
                // Menggunakan FUNGSI BARU untuk mengunggah semua lampiran
