...
useEffect(() => {
    ...
    <div className="flex items-center gap-1 mb-1">
        <label htmlFor="referensiSurat" className="font-semibold text-sm flex-shrink-0">
            Ref. Surat:
        </label>
        {isPrintMode ? (
            <span className="font-normal text-xs ml-2">{form.referensiSurat || "-"}</span>
        ) : (
            <input
                type="text"
                name="referensiSurat"
                ...
                placeholder="No. Surat Referensi (Opsional)"
                className="w-full p-1 border border-gray-300 rounded text-xs select-input-only"
            />
        )}
    </div>
