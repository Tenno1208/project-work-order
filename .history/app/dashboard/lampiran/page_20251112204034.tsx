...
export default function DataPengajuanPage() {
  ...
  const router = {
    push: (path: string) => {
      ...
      // In a real Next.js app, this would change the route.
      // Here, we just log and can perhaps simulate a temporary message.
  ...
  // --- Fungsi untuk Mengambil Data dari API dengan Exponential Backoff ---
  useEffect(() => {
      ...
      for (let i = 0; i < MAX_RETRIES; i++) {
        try {
          ...
          if (i < MAX_RETRIES - 1) {
            ...
            const finalErrorMsg = "Gagal total memuat data pengajuan. Pastikan server API aktif dan mengembalikan data yang benar.";
  ...
  if (error) {
    return (
      <div className="p-10 text-center bg-white rounded-xl shadow-lg border-red-300 border-2 my-10 max-w-2xl mx-auto">
        ...
        <p className="text-sm text-gray-500 mt-4">Silakan cek status API Anda di URL: <a href={API_URL} target="_blank" className="text-blue-600 underline break-all">{API_URL}</a></p>
      </div>
    );
  }
