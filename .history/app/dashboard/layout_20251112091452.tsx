...
useEffect(() => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const res = await fetch("https://gateway.pdamkotasmg.co.id/api-gw-balanced/portal-pegawai/api/auth/me", {
        headers: {
      ...
      if (!res.ok) {
        console.error("Gagal fetch user data:", res.status);
        setLoading(false);
        return;
      }
      ...
      if (result?.data?.user) {
        setUserData({
          nama: result.data.user.name,
          npp: result.data.user.npp,
        });
      }
    } catch (err) {
