...
export default function DataPengajuanPage() {
    ...
    const [authError, setAuthError] = useState<string | null>(null); // State untuk error otorisasi
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const headers: HeadersInit = {
                ...
                // 2. Kirim token sebagai header Authorization
            ...
            for (let i = 0; i < MAX_RETRIES; i++) {
                try {
                    ...
                    const response = await fetch('/api/pengajuan', {
                        method: 'GET',
                        headers: headers, // Gunakan headers yang sudah ada token
                    }); 
                    ...
                    if (error.message.includes("Otorisasi Gagal")) {
                         ...
                         setLoading(false);
                         return; // Berhenti jika otorisasi gagal
                    }
                    ...
                    if (i < MAX_RETRIES - 1) {
                        ...
                        setLoading(false);
                        setPengajuans([
                            { id: 999, tanggal: "00-00-0000", hal: "GAGAL MEMUAT DATA ASLI (Jaringan/Server)", pelapor: "System Error", status: "Error" },
                        ]);
                    }
                }
            }
    ...
    // ... sisa Action Handlers dan Filtering tetap sama ...
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 text-gray-800 bg-gray-50 min-h-screen font-sans">
            ...
            {/* Tambahkan peringatan otorisasi di sini */}
            {authError && (
                <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-md" role="alert">
                    <div className="flex items-center">
                        <AlertTriangle size={20} className="mr-3 flex-shrink-0" />
                        <p className="font-bold">Kesalahan Otorisasi!</p>
                    </div>
                    <p className="ml-7 text-sm">{authError} Silakan periksa login Anda.</p>
                </div>
            )}
