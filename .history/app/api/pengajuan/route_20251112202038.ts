...
// Bertanggung jawab mengupload file ke file handler PDAM dengan nama kustom yang unik.
...
export async function POST(req: Request) {
    try {
        ...
        // A. Cari dan Upload Tanda Tangan (TTD harus diupload sequential karena URL-nya langsung dibutuhkan)
        const ttdFile = formData.get('ttdPelapor');
        if (ttdFile instanceof File) {
            const ttdExt = ttdFile.name.split('.').pop();
            // NAMA UNIK UNTUK TANDA TANGAN: work-order_{timestamp}.{ext}
            const ttdCustomName = `work-order_${timestamp}.${ttdExt}`; 
            ttdUrl = await uploadFile(ttdFile, token, ttdCustomName, 'ttd'); 
        }

        // B. Cari dan Upload File Lampiran (dapat dilakukan paralel)
        let fileIndex = 0;
        while (true) {
            const file = formData.get(`file${fileIndex}`); 
            if (file instanceof File) {
                const fileExt = file.name.split('.').pop();
                // NAMA UNIK UNTUK LAMPIRAN: work-order_{timestamp}_{index+1}.{ext}
