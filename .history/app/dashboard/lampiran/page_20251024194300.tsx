...
          body { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          /* Saat dicetak, select box tampil seperti teks biasa */
          .field-box select {
            appearance: none;
...
        }
        ...
        /* PERBAIKAN: Berikan border dan background putih untuk select box di mode edit (non-cetak) */
        .edit .form-print-area select {
          border: 1px solid #ccc; /* Border yang terlihat */
...
                  <select 
                    ...
                    className="w-full"
                    style={{ borderBottom: 'none' }} // Untuk menghindari duplikasi border-bottom
                  >
...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Tambahkan UI untuk Upload Foto */}
              <div className="border p-4 rounded-lg">
                <div className="flex items-center gap-2 text-sm font-semibold text-gray-700">
                  <UploadCloud size={20} className="text-blue-500" /> Upload Foto Kerusakan
                </div>
                <input
                  type="file"
                  accept="image/*"
                ...
                />
                {photoFile && <p className="text-xs text-gray-600 mt-1">File: {photoFile.name}</p>}
                <button
                  onClick={handleUploadPhoto}
                  disabled={!photoFile || photoProgress > 0}
                  className="mt-3 w-full bg-green-500 text-white py-2 rounded-lg text-sm hover:bg-green-600 disabled:bg-gray-400"
                >
                  {photoProgress > 0 && photoProgress < 100 ? `Mengunggah... ${photoProgress}%` : photoUrl ? "Foto Berhasil Diunggah!" : "Upload Foto"}
                </button>
                {photoUrl && <p className="text-xs text-green-600 mt-1 break-all">URL Foto: {photoUrl}</p>}
              </div>

              {/* Tambahkan UI untuk Upload Dokumen */}
              <div className="border p-4 rounded-lg">
                <div className="flex items-center gap-2 text-sm font-semibold text-gray-700">
                  <UploadCloud size={20} className="text-blue-500" /> Upload Dokumen Pendukung
                </div>
                <input
                  type="file"
                  ...
                  className="mt-2 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                />
                {docFile && <p className="text-xs text-gray-600 mt-1">File: {docFile.name}</p>}
                <button
                  onClick={handleUploadDoc}
                  disabled={!docFile || docProgress > 0}
                  className="mt-3 w-full bg-green-500 text-white py-2 rounded-lg text-sm hover:bg-green-600 disabled:bg-gray-400"
                >
                  {docProgress > 0 && docProgress < 100 ? `Mengunggah... ${docProgress}%` : docUrl ? "Dokumen Berhasil Diunggah!" : "Upload Dokumen"}
                </button>
                {docUrl && <p className="text-xs text-green-600 mt-1 break-all">URL Dokumen: {docUrl}</p>}
              </div>
            </div>
            {uploadError && <div className="mt-4 p-3 bg-red-100 text-red-700 rounded text-sm">{uploadError}</div>}
          </div>
        </div>
      </div>
    </div>
