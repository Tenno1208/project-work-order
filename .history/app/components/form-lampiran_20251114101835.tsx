...
// --- KONSTANTA API DARI ENV ---
// Menggunakan API_BASE_URL yang ditentukan di file edit.
const API_BASE_URL = "https://bright-chicken-knock.loca.lt";
const POST_API_PENGAJUAN_URL = `${API_BASE_URL}/api/pengajuan`;
const MAX_RETRIES = 3; 
// API Route lokal untuk update tidak diperlukan karena kita menggunakan POST_API_PENGAJUAN_URL dengan method PUT
// untuk update data di API eksternal.
...
interface FormLampiranProps {
    initialData: FormDataState; // Data yang sudah di-fetch dari halaman Edit
    isEditMode: boolean;
    uuid: string;
    halOptions: HalOption[];
    satkers: SatkerDef[];
    nomorSurat: string | null;
    existingFilePaths: string[];
    ttdPelaporPreviewUrl: string | null;
...
export default function FormLampiranBase({
    initialData, 
    isEditMode, 
    uuid,
    halOptions,
    satkers,
    nomorSurat,
    existingFilePaths: initialExistingFilePaths,
    ttdPelaporPreviewUrl: initialTtdPelaporPreviewUrl,
    setNotification,
    onSubmissionSuccess,
}: FormLampiranProps) {
    ...
    const [existingFilePaths, setExistingFilePaths] = useState<string[]>(initialExistingFilePaths);
    
    // TTD state diproses untuk transparansi jika di mode Edit
    const [ttdPelaporPreview, setTtdPelaporPreview] = useState<string | null>(null);

    const [files, setFiles] = useState<File[]>([]);
    const [isPrintMode, setIsPrintMode] = useState(false);
    const [previews, setPreviews] = useState<string[]>(initialExistingFilePaths);
    ...
    // Effect untuk memproses TTD menjadi transparan setelah initialData ada
    useEffect(() => {
        const processTtd = async () => {
            if (initialTtdPelaporPreviewUrl) {
                const transparentUrl = await makeImageTransparent(initialTtdPelaporPreviewUrl);
    ...
    }, [isEditMode, initialTtdPelaporPreviewUrl]);
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const previewsNew = [...existingFilePaths, ...selectedFiles.map((file) => URL.createObjectURL(file))];
        setPreviews(previewsNew);
    };

    const handleRemoveFile = (index: number) => {
        // Hapus dari daftar path file lama/baru
        if (index < existingFilePaths.length) {
            setExistingFilePaths(prev => prev.filter((_, i) => i !== index));
        } else {
            const fileIndex = index - existingFilePaths.length;
    ...
    const proceedSubmission = useCallback(async () => {
        ...
        try {
            ...
            // 2. Tambahkan file lampiran baru
            ...
            if (isEditMode && uuid) {
                // UPDATE menggunakan POST/PUT ke endpoint utama, sertakan UUID
                submitUrl = POST_API_PENGAJUAN_URL; 
            ...
            for (let i = 0; i < MAX_RETRIES; i++) {
                try {
                    ...
                    if (finalResult.success) {
                        // Panggil callback sukses dari parent
                        onSubmissionSuccess(); 
                        return;
                    }
    ...
    }, [form, files, ttdPelaporFile, isEditMode, uuid, existingFilePaths, onSubmissionSuccess, setNotification]);


    const handleAjukan = async () => {
        // ... (Logic Validasi dan Modal, lalu panggil proceedSubmission)
        const wajibDiisi = [
            { field: "hal", label: "Hal" }, { field: "kepada", label: "Kepada" },
            { field: "satker", label: "Satker" }, { field: "kodeBarang", label: "Kode Barang" },
            { field: "keterangan", label: "Keterangan" }, { field: "pelapor", label: "Nama Pelapor" },
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="font-semibold text-base">
                                {isEditMode ? `Edit Pengajuan (UUID: ${uuid?.substring(0, 8)}...)` : "Lampiran Pengajuan Perbaikan"}
                            </div>
                            ...
                            {isPrintMode ? (
                                ...
                                <div className="select-input-only">
                                    <Select
                                        ...
                                        onChange={(option) => setForm((f) => ({ ...f, kepada: option ? option.value : "" }))}
                                        options={satkers.map((s) => ({ value: s.label, label: s.label }))}
                                        placeholder="Cari atau pilih tujuan..."
                                        className="text-sm w-64"
                                        styles={{ menu: (base) => ({ ...base, zIndex: 50, position: "absolute" }) }}
                                        menuPlacement="auto"
                                    />
                                </div>
                            )}
                            ...
                            {isPrintMode ? (
                                ...
                                <div className="select-input-only">
                                    <Select
                                        ...
                                        onChange={(option) => setForm((f) => ({ ...f, satker: option ? option.value : "" }))}
                                        options={satkers.map((s) => ({ value: s.id, label: s.label }))}
                                        placeholder="Cari atau pilih satker..."
                                        className="text-sm w-full"
                                        styles={{ menu: (base) => ({...base, zIndex: 100, position: "absolute", marginTop: 2, }) }}
                                ...
                                {previews.map((src, i) => (
                                    <div key={i} className="relative group">
                                        ...
                                        <p className="text-xs mt-1 text-gray-500 truncate">
                                            {i < initialExistingFilePaths.length ? `File Lama #${i+1}` : `File Baru #${i+1}`}
                                        </p>
                                    </div>
                                ))}
                            </div>
