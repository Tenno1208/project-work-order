...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Fungsi untuk menentukan icon yang tepat berdasarkan halaman
  const getPageIcon = () => {
    // Jika di halaman lampiran atau spk (termasuk sub-halaman riwayat)
    if (pathname.includes("/dashboard/lampiran") || pathname.includes("/dashboard/spk")) {
      return FileText;
    }
    // Untuk halaman lain, gunakan icon dari halaman tersebut
    return currentPage.icon;
  };
          ...
          <div className="relative z-10 px-4 lg:px-8 py-6 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4 ml-12 lg:ml-0">
              <div className="bg-white/20 backdrop-blur-md p-4 rounded-2xl shadow-xl border border-white/30 hover:scale-105 transition-transform">
                {/* Menggunakan icon yang telah disesuaikan */}
                {(() => {
                  const PageIcon = getPageIcon();
                  return (
                    <PageIcon className="text-white drop-shadow-md" size={30} strokeWidth={2.5} />
                  );
                })()}
              </div>
              <div>
