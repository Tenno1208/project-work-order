...
  inputValue,
  setInputValue,
  onSelect, // (pegawaiObject) => { handleAddPerson(pegawaiObject); }
  pegawaiList,
  assignedPeople,
  isLoadingPegawai,
...
  // 1. Filter pegawai berdasarkan input.
  // 2. Filter pegawai yang TIDAK ADA di assignedPeople.
  const filtered = pegawaiList
    .filter(
      (p) =>
        p.name &&
        p.name.toLowerCase().includes((inputValue || "").toLowerCase()) &&
        // Cek apakah NPP/Nama sudah ada di assignedPeople
        !assignedPeople.some((a) => (a.npp && a.npp === p.npp) || a.name === p.name)
    )
    .slice(0, 10);
...
        // 💡 PERUBAHAN: Kirim objek pegawai, bukan hanya nama
        onSelect(pick); 
        setInputValue("");
        setOpen(false);
      }
...
          setOpen(true);
        }}
        onFocus={() => setOpen(true)} // Tampilkan dropdown saat fokus
        onKeyDown={handleKey}
...
// 💡 PERUBAHAN: Menampilkan nama dan NPP (jika ada) di chip.
...
        </svg>
        {/* Show Nama (NPP) */}
        {/* 💡 PERUBAHAN: Tampilkan nama lengkap + NPP */}
...
  // Format: { name: string, npp: string/null, jabatan: string/null }
  const [pegawaiList, setPegawaiList] = useState([]); 
  const [isLoadingPegawai, setIsLoadingPegawai] = useState(false); // Status loading data pegawai

  // 🔹 FETCH DATA JENIS PEKERJAAN (Tidak ada perubahan)
  useEffect(() => {
...
  // 🔹 FETCH DATA SEMUA PEGAWAI DARI API EKSTERNAL (Tidak ada perubahan)
  useEffect(() => {
...
  // 💡 PERUBAHAN: Menerima objek pegawai jika dari dropdown, atau mencari sendiri jika dari input manual.
  const handleAddPerson = (personFromDropdown = null) => {
    let name;
    let npp = null;
    let detail;

    if (personFromDropdown) {
        // Jika dari dropdown, gunakan data yang sudah lengkap
        name = personFromDropdown.name;
        npp = personFromDropdown.npp;
    } else {
        // Jika dari Enter/Tombol Add (input manual)
        name = currentPersonInput.trim();
        detail = getPersonDetail(name);
        npp = detail.npp; // Ambil NPP jika ketemu
    }

    if (!name) return;

    // Cek apakah nama (atau NPP jika ada) sudah ada
    const alreadyAssigned = data.assignedPeople.some(p => 
        p.name === name || (p.npp && p.npp === npp)
    );
...
    const newPerson = {
        name: name,
        npp: npp, // Menggunakan NPP dari dropdown/pencarian
        isPic: isFirstPerson, 
    };
...
    }));
    setCurrentPersonInput(""); // Selalu kosongkan input setelah menambah
  };
...
            ...p,
            isPic: index === 0 ? true : false, // Set orang pertama sebagai PIC baru, yang lain FALSE
        }));
    }
...
    // Jika Enter ditekan saat input terisi, AutocompletePerson akan memanggil onSelect (jika ada saran)
    // Jika tidak ada saran, Enter akan ditangani di sini.
    if (e.key === 'Enter') {
      e.preventDefault(); // Mencegah form submit
      // Panggil handleAddPerson tanpa argumen (artinya input manual)
      handleAddPerson();
    }
...
  };
  
  // --- LOGIKA API MENUGASKAN (Tidak ada perubahan signifikan) ---
...
  // --- KOMPONEN BANTUAN (Tidak ada perubahan signifikan) ---
...
        .map(p => `${p.name} ${p.npp ? `(${p.npp})` : ''} ${p.isPic ? '(PIC)' : ''}`)
        .join(', ');
...
                          onSelect={(personObject) => {
                            // 💡 PERUBAHAN: Langsung panggil handleAddPerson dengan objek dari dropdown
...
                        <Button 
                          onClick={() => handleAddPerson()} // Jika tombol +, panggil dengan input manual
