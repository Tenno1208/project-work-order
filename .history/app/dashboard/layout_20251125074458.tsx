...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const fetchAndStorePermissions = async (token: string) => {
        try {
            ...
            const permissions: string[] = Array.isArray(json.data) 
                ? json.data.map((p: any) => p.name || p) 
                : [];
        ...
        const loadInitialData = async () => {
            ...
            if (storedUserData) {
                try {
                    ...
                    profileData = null; // Jika gagal parse, anggap tidak ada
                }
            }
            ...
            if (storedPermissions) {
                try {
                    ...
                    permissionsData = null; // Jika gagal parse, anggap tidak ada
                }
            }
            
            // 3. Jika Data User TIDAK ADA di localStorage, Panggil API
            ...
            // 4. Jika Permissions TIDAK ADA di localStorage, Panggil API
            if (!permissionsData || permissionsData.length === 0) {
                console.log("⏳ Permissions not found in localStorage, fetching from API...");
    ...
    // ... sisa kode menuItems, getCurrentPage, toggleExpand, handleNavigation, handleLogout ...
    ...
    const handleLogout = async () => {
        ...
        // Hapus data lokal sebelum redirect
        localStorage.removeItem("token"); 
        localStorage.removeItem("user_data");
        localStorage.removeItem("user_permissions");
