...
export default function RiwayatDataPengajuan() {
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto"> {/* Reduced vertical spacing */}
      ...
      {/* Tabel - Reduced padding and font size */}
      <div className="bg-white rounded-2xl shadow-xl border border-blue-100 overflow-hidden"> {/* rounded-3xl -> rounded-2xl */}
        <div className="px-5 py-4 border-b border-blue-100"> {/* px-8 py-6 -> px-5 py-4 */}
          <h3 className="text-lg font-bold text-black"> {/* text-xl -> text-lg */}
            Daftar Riwayat Pengajuan
          </h3>
          <p className="text-black text-xs mt-1"> {/* text-sm -> text-xs */}
          ...
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-blue-50">
              {filteredData.map((item, index) => (
                ...
                >
                  <td className="px-4 py-3 text-xs font-medium text-black">{item.id}</td> {/* Reduced padding/size */}
                  <td className="px-4 py-3 text-xs text-black whitespace-nowrap"> {/* Reduced padding/size */}
                    {new Date(item.tanggalPengajuan).toLocaleDateString(
                      "id-ID",
                      { year: "numeric", month: "short", day: "numeric" } // Shortened month name
                    )}
                  </td>
                  <td className="px-4 py-3 text-xs text-black">{item.namaPengaju}</td> {/* Reduced padding/size */}
                  <td className="px-4 py-3 text-xs text-black whitespace-nowrap">{item.jenisPengajuan}</td> {/* Reduced padding/size */}
                  <td className="px-4 py-3 whitespace-nowrap"> {/* Reduced padding/size */}
                    <span
                  ...
                  <td className="px-4 py-3 text-xs text-black max-w-sm truncate"> {/* Reduced padding/size, max-w-xs -> max-w-sm */}
                    {item.keterangan}
                  </td>
                  <td className="px-4 py-3 text-xs font-medium"> {/* Reduced padding/size */}
                    <div className="flex gap-1.5"> {/* Reduced gap */}
                      <button
                        className="p-1.5 bg-blue-100 hover:bg-blue-200 rounded-md transition-colors duration-200" // Reduced padding/rounded
                        title="Lihat Detail"
                      >
                        <Eye
                      ...
                      <button
                        className="p-1.5 bg-green-100 hover:bg-green-200 rounded-md transition-colors duration-200" // Reduced padding/rounded
        ...
        {filteredData.length === 0 && (
          <div className="px-5 py-8 text-center"> {/* Reduced padding */}
            <History className="mx-auto text-black mb-3" size={40} /> {/* Reduced size */}
            <p className="text-black text-base font-medium">
              Tidak ada data riwayat yang ditemukan
            </p>
            <p className="text-black text-xs mt-1"> {/* Reduced size */}
      ...
      <div className="bg-gradient-to-r from-blue-50 to-white rounded-xl p-4 border border-blue-100"> {/* p-6 -> p-4, rounded-2xl -> rounded-xl */}
        <div className="flex items-center justify-between">
          <div className="text-xs text-black"> {/* text-sm -> text-xs */}
            <p className="font-medium">Informasi Tambahan:</p>
            <ul className="mt-1 space-y-0.5 list-disc list-inside"> {/* Reduced margins/spacing */}
          ...
          <div className="text-right">
            <p className="text-xs text-black">Terakhir diperbarui:</p> {/* text-sm -> text-xs */}
            <p className="text-base font-bold text-black"> {/* text-lg -> text-base */}
