...
// Mengganti penggunaan router.push dengan window.location.href untuk simulasi navigasi
// NOTE PENTING: Menambahkan console.log untuk memastikan fungsi klik berjalan.
const navigateTo = (href) => {
  console.log(`[NAVIGASI] Mencoba mengalihkan ke: ${href}`);
  // Inilah perintah yang seharusnya memuat halaman baru
...
export default function DaftarSPKPage() {
  ...
  // NOTE: Router mock telah dihapus dan diganti dengan fungsi navigateTo di atas.
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 p-4"> 
      <div className="max-w-7xl mx-auto space-y-4"> 
        ...
        <div className="bg-white rounded-2xl shadow-lg p-5 border border-blue-100"> 
          <div className="flex items-start justify-between mb-4"> {/* Adjusted for button alignment */}
            ...
            {/* NEW BUTTON: Buat SPK Baru */}
            <button
              onClick={() => navigateTo('/dashboard/spk/format')}
              className="bg-gradient-to-r from-teal-500 to-green-600 text-white px-5 py-2 text-sm rounded-xl font-medium hover:shadow-xl transition-all shadow-md hover:scale-[1.02] mt-2 md:mt-0"
            >
              + Buat SPK Baru
            </button>
            {/* END NEW BUTTON */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4"> 
            {Object.entries(statusCount).map(([key, value]) => (
              <div
                key={key}
                className={`${getStatusColor(key === 'semua' ? 'Semua' : key.charAt(0).toUpperCase() + key.slice(1))} rounded-xl p-3 text-white shadow-md hover:scale-[1.02] transition-transform`}
              >
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredSpk.map((spk, index) => (
                    <tr
                      ...
                      onClick={() => navigateTo(`/dashboard/spk/${spk.id}`)}
                    >
                      ...
                      <td className="px-4 py-3 text-center"> 
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            navigateTo(`/dashboard/spk/${spk.id}/edit`);
                          }}
                          className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-3 py-1.5 text-xs rounded-lg font-medium hover:shadow-lg hover:scale-[1.05] transition-all"
                        >
                          Lihat Detail
                        </button>
                      </td>
