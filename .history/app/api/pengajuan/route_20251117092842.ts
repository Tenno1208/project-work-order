...
async function uploadFile(file: File, token: string, customName: string, type: 'ttd' | 'pengajuans'): Promise<string> {
    ...
    // --- START PERBAIKAN LOGIKA UTAMA ---
    
    // API File Handler PDAM sukses jika:
    // 1. Status HTTP adalah OK (res.ok = 200-299)
    // 2. Body memiliki properti data.fileUrl (berdasarkan screenshot Postman)
...
// ... (Fungsi POST(req: Request) lainnya)
