...
// URL DELETE SPK eksternal: ${API_BASE_URL}/api/spk/delete{uuid}
// Kita akan memasukkan nilai UUID ke dalam string ini di bawah.
const DELETE_API_SPK_URL_TEMPLATE = `${API_BASE_URL}/api/spk/delete`; 

/**
 * Handle DELETE request untuk menghapus SPK dari API eksternal.
 * Rute lokal: /api/spk/delete/[uuid]
 */
export async function DELETE(
    req: Request, 
    { params }: { params: { uuid: string } }
) {
    // Menggunakan UUID dari parameter rute
    const { uuid } = params; 
    ...
    // PERBAIKAN PENTING: Membangun URL DELETE eksternal dengan UUID
    const externalDeleteUrl = `${DELETE_API_SPK_URL_TEMPLATE}${uuid}`; 
    
    console.log(`[PROXY INFO] Menghubungi API DELETE SPK: ${externalDeleteUrl}`);
    ...
    try {
        ...
        if (!res.ok) {
            console.error(`[PROXY ERROR] Gagal hapus SPK (Status ${res.status}). Respon: ${rawText.slice(0, 200)}`);
            let errorJson = null;
        ...
        // Mengembalikan respons sukses dari server eksternal
        return NextResponse.json(json, { status: 200 });

    } catch (error) {
        console.error("Error API /spk/delete/[uuid]:", error);
        return NextResponse.json(
            { success: false, message: "Kesalahan server internal Next.js." },
            { status: 500 }
        );
    }
}