...
export default function LampiranPengajuanPage() {
  ...
  // 🔹 Tambahan untuk tanda tangan pelapor
  ...
  // 🔹 Handler upload tanda tangan Pelapor + hapus background putih
  const handleTtdPelaporChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Proses transparansi otomatis
    const transparentUrl = await makeImageTransparent(previewUrl);
    setTtdPelaporPreview(transparentUrl);
  };

  // 🔹 Fungsi hapus background putih jadi transparan
  // 🔹 Fungsi hapus background putih atau hitam → jadi transparan
async function makeImageTransparent(imgUrl: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // Batas warna untuk putih dan hitam
      const whiteThreshold = 235; // makin rendah, makin sedikit putih dihapus
      const blackThreshold = 35;  // makin tinggi, makin banyak hitam dihapus
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        // Jika terlalu terang (putih) atau terlalu gelap (hitam) → transparan
                ...
                <div className="mt-3 text-center">
                  ...
                  {ttdPelaporPreview && (
                    <img
                      ...
                      className="mx-auto mt-2 w-40 h-auto object-contain border border-gray-300 rounded"
                    />
                  )}
                </div>
              )}

              {/* Saat print tampil tanda tangan */}
