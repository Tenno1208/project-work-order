...
// Mengambil URL dasar API dari variabel environment
const API_BASE_URL = process.env.GET_API_HAL_DROPDOWN;
...
export async function GET(req: Request) {
  try {
    ...
    // Menggunakan URL yang diambil dari env
    const res = await fetch(EXTERNAL_HAL_API, { 
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${token}`,
        "ngrok-skip-browser-warning": "true", // Pertahankan jika diperlukan
      },
      cache: "no-store",
    });
