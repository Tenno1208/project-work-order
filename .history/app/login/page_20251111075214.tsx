...
// ✅ PERBAIKAN 1: Gunakan `next/navigation` untuk App Router
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await response.json(); // Langsung parse JSON, Next.js API biasanya selalu mengirim JSON

      console.log("Response dari /api/login:", data);

      if (!response.ok) {
        // ✅ PERBAIKAN 2: Gunakan pesan dari backend secara langsung
        // Backend kita sudah menyiapkan pesan yang aman dan informatif.
        // Kita cukup mengambilnya dari `data.message`.
        // Jika `data.message` tidak ada, gunakan pesan default.
        const errorMessage = data.message || "Terjadi kesalahan saat login. Silakan coba lagi.";
        throw new Error(errorMessage);
      }

      // Jika berhasil, ambil token
      ...
      // alert("Login berhasil!"); // Opsional, bisa dihapus agar lebih smooth
  ...
  // ... (sisanya kode JSX tidak berubah)
  return (
