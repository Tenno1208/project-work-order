...
  file: string | null;
  status: string;
  name: string | null; // Akan digunakan sebagai 'pelapor'
  npp: string | null;
  mengetahui: string | null;
  is_deleted: number;
  created_at: string; // Akan digunakan untuk 'tanggal'
  updated_at: string;
};
...
// Definisi Tipe data Pengajuan yang digunakan untuk state komponen (sudah disesuaikan)
type Pengajuan = {
  id: number;
  tanggal: string;
  hal: string;
...
/**
 * Mengubah string ISO date menjadi format DD-MM-YYYY.
 * @param isoString String tanggal ISO (e.g., "2025-11-12T12:52:48.000000Z")
 * @returns String tanggal terformat (e.g., "12-11-2025")
 */
...
    if (!isoString) return '-';
    
    // Membuat objek Date dari string ISO (otomatis menangani timezone)
    const date = new Date(isoString);
    if (isNaN(date.getTime())) return '-';
    
    // Menggunakan Intl.DateTimeFormat untuk format yang aman dan benar
    return new Intl.DateTimeFormat('id-ID', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    }).format(date).replace(/\//g, '-'); // Mengganti separator / menjadi -
    
  } catch (e) {
    return 'Format Salah';
  }
...
// URL API yang akan digunakan (menggunakan URL yang Anda berikan)
// Catatan: ngrok URL sering kedaluwarsa. Pastikan URL ini masih aktif!
...
      // Menggunakan custom modal/alert karena window.alert() tidak diizinkan di Canvas
...
  // STATE BARU: Untuk menyimpan dan menampilkan pesan kesalahan API di UI
  const [apiError, setApiError] = useState<string | null>(null);

  // --- Fungsi untuk Mengambil Data dari API dengan Exponential Backoff dan MAPPING ---
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      setApiError(null); // Reset error status sebelum fetch baru
      setPengajuans([]); // Clear previous data
      
      let lastError: Error | null = null; // Menyimpan error terakhir
...
            
          if (!response.ok) {
            // Jika response tidak OK (misalnya 4xx atau 5xx), lempar error untuk retry
            throw new Error(`[HTTP ${response.status}] Gagal memuat data. Cek: ${API_URL}`);
          }

          const result: ApiResponse = await response.json();

          // VALIDASI STRUKTUR API
          if (!result.success || !result.data || !Array.isArray(result.data)) {
                // Ini masih dianggap berhasil jika array data kosong
                if (result.data && Array.isArray(result.data) && result.data.length === 0) {
                    console.log("API Success=true, but data array is empty.");
                } else {
                    throw new Error("Struktur respons API tidak valid: 'success' false atau 'data' bukan array.");
                }
          }

          // MAPPING DATA DARI API KE TIPE Pengajuan KOMPONEN
          const mappedData: Pengajuan[] = result.data.map(item => ({
            id: item.id,
            tanggal: formatDate(item.created_at), // Menggunakan helper untuk format tanggal
            // Menggunakan 'keterangan' sebagai 'hal' (subjek), fallback ke 'catatan'
            hal: item.keterangan || item.catatan || 'Tidak Ada Keterangan', 
            // Menggunakan 'name' sebagai 'pelapor', fallback ke 'npp', lalu 'Anonim'
            pelapor: item.name || item.npp || 'Anonim', 
            status: item.status || 'Pending', // Default status jika kosong
          }));
...
        } catch (error: any) {
            lastError = error; // Simpan error ini
...
      }
      ...
      if (lastError) {
          ...
          // Set error message ke state untuk ditampilkan di UI
          setApiError(lastError.message); 
      }
    };
...
                </td>
              </tr>
            ) : apiError ? ( // TAMPILKAN KESALAHAN JIKA ADA
              <tr>
                <td colSpan={6} className="py-10 text-center text-red-700 bg-red-50 font-semibold border-t-4 border-red-300">
                  <div className="flex flex-col items-center justify-center p-4">
                      <span className="text-xl mb-2">❌ GAGAL MEMUAT DATA API</span>
                      <p className="text-sm text-red-600 max-w-lg overflow-x-auto break-all">
                            Pesan Error Terakhir: 
                            <span className="font-normal block mt-1 p-2 bg-red-100 rounded-lg text-xs italic">
                                {apiError}
                            </span>
                        </p>
                      <p className="text-xs mt-3 font-medium text-red-700">
                            **SOLUSI:** Pastikan **ngrok** Anda aktif dan URL API tersebut ({API_URL}) dapat diakses.
                        </p>
                  </div>
                </td>
              </tr>
