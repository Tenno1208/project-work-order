        ...
        const fetchUserData = async () => {
            try {
                ...
                if (!token) {
                    ...
                    router.push("/login");
                    // Hapus user_data jika token tidak ada
                ...
                if (!res.ok) {
                    ...
                    if (res.status === 401 || res.status === 403) {
                        ...
                        // Hapus user_data saat token kadaluarsa
                        localStorage.removeItem("user_data"); 
                        router.push("/login");
                    }
                    setLoading(false);
                ...
                // #######################################################################
                // PERBAIKAN UTAMA: SIMPAN NPP DAN NAMA DI localStorage
                // Key yang digunakan adalah 'user_data' dan NPP ada di properti 'npp'
                // #######################################################################
...
// ... (Bagian bawah kode handleLogout, render, dll. tetap sama)
