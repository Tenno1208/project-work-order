...
    
    // 2. Tambahkan setiap file (photo_i) dan nama file (filename_i)
    for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const fileExt = file.name.split('.').pop() || 'jpg';
        const fileName = `${customNamePrefix}-${i}.${fileExt}`;
        
        // A. Tambahkan file dengan key photo_1, photo_2, dst.
        formData.append(`photo_${i + 1}`, file, fileName);
        
        // B. Tambahkan nama file dengan key filename_1, filename_2, dst. (SOLUSI)
        // Nilainya adalah filepath lengkap (path + filename)
...
        // #################################################
        const newFiles: File[] = [];
        for (let i = 0; i < 4; i++) { // Maksimum 4 file, sesuai logika frontend
...
            
            try {
                // A. Jika hanya ada SATU file, gunakan API lama (FILE_HANDLER_URL)
...
                } 
                // B. Jika ada DUA atau lebih file, gunakan API multiple upload
                else {
...
        const rawText = await res.text();
        ...
        console.log(`[RAW RESPONSE PDAM] Status: ${res.status}. Body: ${rawText.slice(0, 500)}`);
...
return new NextResponse(rawText, {
            status: res.status,
            // Asumsi PDAM mengembalikan JSON, kita set Content-Type JSON
            headers: { 'Content-Type': 'application/json' },
        });
        try {
