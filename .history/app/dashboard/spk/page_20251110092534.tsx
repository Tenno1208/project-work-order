...
            <div className="flex">
              <div className="w-[120px] mt-2">Menugaskan Sdr:</div> {/* Lebar disesuaikan */}
              <div className="flex flex-col gap-2 flex-1">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex items-center gap-1.5"> {/* gap dikurangi */}
                    <div className="w-[10px]">{i}.</div>
                    <select
                      value={data[`menugaskan${i}` as keyof typeof data]}
                      onChange={(e) => updateField(`menugaskan${i}`, e.target.value)}
                      // Styling untuk menampilkan hanya garis bawah
                      className="border-b border-black outline-none bg-white text-sm py-0.5 w-full cursor-pointer"
                      style={{ appearance: 'none', borderRadius: 0 }}
                    >
                      <option value="">{data[`menugaskan${i}` as keyof typeof data] ? data[`menugaskan${i}` as keyof typeof data] : '--- Pilih nama dari daftar ---'}</option>
                      {daftarNama.map((nama) => (
...
            {!showDetail && (
              <Button  onClick={() => setShowDetail(true)} className="mt-4">Selesai</Button>
            )}
          </div>
