...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Kita langsung meneruskan header Authorization apa adanya dari klien
    // (yang diharapkan sudah berformat "Bearer [token]")
    const apiResponse = await fetch(EXTERNAL_API_URL, {
      method: 'GET',
      headers: {
        // Meneruskan header Authorization persis seperti yang diterima dari klien
