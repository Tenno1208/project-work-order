...
// --- KONSTANTA API BARU DARI ENV (Diasumsikan di-fetch via NextJS API Routes atau langsung jika ada NEXT_PUBLIC_) ---
// Karena ini adalah kode client-side, kita harus menggunakan NEXT_PUBLIC_ atau asumsi API_BASE_URL
// Saya akan menggunakan konstanta yang DITURUNKAN dari ENV Anda untuk akses fetch eksternal.
// Catatan: Jika Anda ingin ini diakses langsung di sisi klien, pastikan ENV Anda berprefix NEXT_PUBLIC_
const API_BASE_URL = "https://bright-chicken-knock.loca.lt";

// Endpoint API yang Diperbarui sesuai format ENV yang Anda berikan
const POST_API_PENGAJUAN_URL = `${API_BASE_URL}/api/pengajuan`;
// Menggunakan format EDIT_API_PENGAJUAN_URL untuk detail dan update
const DETAIL_UPDATE_API_PATH_BASE = `${API_BASE_URL}/api/pengajuan/edit`; 
const HAL_API_PATH = `${API_BASE_URL}/api/hal`;
const MAX_RETRIES = 3; 

// --- TYPES BARU (Tidak ada perubahan) ---
...
type ApiPengajuanDetail = {
    ...
    kode_barang: string;
    keterangan: string; 
    file: string | null; // Simpan path file, biasanya JSON string/array
    status: string;
    name: string | null; // Pelapor name
    npp: string | null; // Pelapor npp
    mengetahui: string | null;
    npp_mengetahui: string | null;
    ttd_pelapor: string | null; // URL tanda tangan
    created_at: string;
};
...
// --- Notification & Modal Components (Tetap sama) ---
...
// --- HELPER FUNCTION TTD (Tetap sama) ---
...
export default function LampiranPengajuanPage() {
    ...
    const [existingFilePaths, setExistingFilePaths] = useState<string[]>([]); // Untuk file yang sudah ada
    // ---------------------------------

    const [loading, setLoading] = useState(true);
    const [initialLoading, setInitialLoading] = useState(true); // Loading untuk data awal
    ...
    const fetchPengajuanDetail = useCallback(async (token: string, uuid: string, halOptions: HalOption[], satkers: SatkerDef[]) => {
        try {
            // Menggunakan DETAIL_UPDATE_API_PATH_BASE + uuid
            const res = await fetch(`${DETAIL_UPDATE_API_PATH_BASE}/${uuid}`, {
            ...
            if (data.file) {
                try {
                    // Asumsi data.file adalah string JSON dari path file yang sudah ada
                    const existingFiles = JSON.parse(data.file);
                    if (Array.isArray(existingFiles)) {
                        setExistingFilePaths(existingFiles); 
                        setPreviews(existingFiles); 
                    }
                } catch (e) {
                    console.warn("File path tidak bisa di-parse:", data.file);
                }
            }
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            ...
            try {
                // 1. Fetch User Data (Menggunakan API Route lokal)
                ...
                // 2. Fetch Hal Options (Menggunakan API eksternal baru)
                const halRes = await fetch(HAL_API_PATH, { headers, cache: "no-store" });
                ...
                // 3. Fetch Satker Options (Menggunakan API Route lokal)
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Gabungkan file lama (path) dan file baru (preview URL)
        const previewsNew = existingFilePaths.concat(selectedFiles.map((file) => URL.createObjectURL(file)));
        setPreviews(previewsNew);
    };

    const handleRemoveFile = (index: number) => {
        // Hapus dari daftar file baru
        const newFiles = files.filter((_, i) => i !== index - existingFilePaths.length);
        setFiles(newFiles);
        
        // Hapus dari daftar path file lama
        const newExistingPaths = existingFilePaths.filter((_, i) => i !== index);
        setExistingFilePaths(newExistingPaths);

        // Hapus dari daftar preview total
    ...
    const proceedSubmission = useCallback(async () => {
        ...
        try {
            ...
            if (isEditMode && editUuid) {
                submitUrl = `${DETAIL_UPDATE_API_PATH_BASE}/${editUuid}`; // Gunakan EDIT_API_PENGAJUAN_URL
                method = "POST"; // Asumsi server menggunakan POST untuk update di Next.js API Route atau PUT/POST di API eksternal
                // Untuk API eksternal yang di-hardcode ke POST/PUT/PATCH:
                // Kita akan menggunakan PUT sesuai konvensi REST untuk update.
                method = "PUT";
                ...
                submitUrl = POST_API_PENGAJUAN_URL; // Gunakan POST_API_PENGAJUAN_URL
                method = "POST";
            }
    ...
    if (initialLoading) {
        return (
            <div className="flex items-center justify-center min-h-screen bg-gray-100">
                ...
                <span className="text-xl text-gray-700 font-medium">Memuat data {isEditMode ? 'edit' : 'form'}...</span>
            </div>
        );
    }
