...
export default function AdminDashboard() {
    ...
    // State Izin Admin - DISET TRUE SECARA DEFAULT UNTUK MENGHINDARI BLOKIR
    const [isUserAdmin, setIsUserAdmin] = useState(true); 

    // --- FUNGSI API & DATA ---
    
    // Fungsi untuk mendapatkan semua raw permissions dari user yang login (untuk daftar sistem perms)
    ...
    const loadSystemPermissions = useCallback(async (token: string) => {
        ...
        // LOGIKA PERMISSION CHECK DIBAWAH DINONAKTIFKAN (isUserAdmin sudah true)
        
        // Gunakan permission yang dimiliki user yang login sebagai daftar permission yang tersedia
    ...
    useEffect(() => {
        ...
        // 1. Muat izin sistem (yang kini mengabaikan cek admin)
    ...
    useEffect(() => {
        const token = localStorage.getItem('token');
        // Panggil fetchTargetUserPermissions hanya jika user dipilih dan token ada
    ...
    // --- CEK AKSES AWAL ---
    // Karena isUserAdmin selalu true, blokir akses dinonaktifkan.
    if (isLoading) {
        return (
             <div className="p-6">
