...
        // ==========================================================
        // >> PERBAIKAN FINAL: MENGEMBALIKAN RESPONS DENGAN URL YANG DIBERSIHKAN <<
        // ==========================================================
        
        const rawText = await res.text();
        let responseJson: any; // Inisialisasi di sini
...
            // Jika gagal parse, kembalikan status HTTP yang benar
...
        }
        ...
        // Logika 2: Membersihkan URL dari karakter escape yang berlebihan
        if (responseJson?.data) {
            // 1. Bersihkan field 'file' (mengandung \" dan \\\/)
            if (responseJson.data.file) {
                let cleanFile = responseJson.data.file;
                // Hapus tanda kutip ganda di awal dan akhir string URL yang salah
                if (cleanFile.startsWith('"') && cleanFile.endsWith('"')) {
                    cleanFile = cleanFile.substring(1, cleanFile.length - 1);
                }
                // Ganti \\\/ menjadi / (de-escape)
                responseJson.data.file = cleanFile.replace(/\\\//g, '/');
            }
            
            // 2. Bersihkan field 'ttd_pelapor' (mengandung \/)
            if (responseJson.data.ttd_pelapor) {
                // Ganti \/ menjadi / (de-escape)
                responseJson.data.ttd_pelapor = responseJson.data.ttd_pelapor.replace(/\\\//g, '/');
            }
        }
        
        // Logika 3: Mengembalikan respons JSON yang sudah dibersihkan
