...
// Menggunakan DELETE URL eksternal (disesuaikan agar sesuai dengan format ENV Anda: /api/spk/{id}/delete)
...
type SPKItem = {
    id: number;
    nomor: string; // Diisi dari UUID atau ID
    pekerjaan: string; // Diisi dari uraian_pekerjaan
    tanggal: string; 
    status: "Selesai" | "Proses" | "Menunggu" | string;
    uuid: string; // UUID harus ada untuk navigasi
};
...
export default function DaftarSPKPage() {
    ...
    const confirmDeletion = async () => {
        ...
        try {
            // Memanggil API DELETE sesuai template ENV: /api/spk/{id}/delete
            const deleteUrl = `${DELETE_API_SPK_URL_BASE}/${spkId}/delete`; 
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            for (let i = 0; i < MAX_RETRIES; i++) {
                try {
                    // PANGGIL API ROUTE LOKAL: /api/spk/list
                    ...
                    // PERBAIKAN MAPPING DATA API SESUAI STRUKTUR BARU
                    const mappedSpks: SPKItem[] = result.data.map((item: any) => ({
                        id: item.id,
                        // Menggunakan UUID/ID sebagai Nomor Display
                        nomor: item.uuid || item.id?.toString() || "-", 
                        // Menggunakan uraian_pekerjaan sebagai Pekerjaan
                        pekerjaan: item.uraian_pekerjaan || item.jenis_pekerjaan || "Tidak Ada Uraian", 
                        tanggal: item.tanggal || "-",
                        // Mapping status: 'pending' -> 'Menunggu', 'completed' -> 'Selesai'
                        status: item.status === 'pending' ? 'Menunggu' : item.status === 'completed' ? 'Selesai' : item.status || "Menunggu",
                        uuid: item.uuid,
                    }));

                    // Filter item yang tidak memiliki UUID atau ID yang valid (pencegahan)
    ...
    const filteredSpk = spks.filter((item) => {
        // Menggunakan item.nomor dan item.pekerjaan (yang sudah dijamin string saat mapping)
