...
export default function SPKPage() {
  ...
  // ✅ Ambil data dari API dengan token dari localStorage
  useEffect(() => {
    const fetchJenisPekerjaan = async () => {
      try {
        ...
        if (!res.ok) {
          console.error("Gagal ambil data:", res.status, res.statusText);
          return;
        }
        ...
        if (Array.isArray(items)) {
          const mapped = items.map((item: any) => ({
            id: item.id?.toString(),
            nama: item.nama_pekerjaan || item.nama || "Tanpa Nama",
          }));
          setJenisPekerjaan(mapped);
        } else {
          console.warn("Format data API tidak dikenali:", json);
        }
      } catch (err) {
        console.error("Gagal ambil jenis pekerjaan:", err);
      }
    };
